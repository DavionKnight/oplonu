...
//const uchar   NetOurEther[6]={0x00,0x00,0x5B,0xEE,0xDE,0xAE}; /* Our ethernet address */
...
void ArpTimeoutCheck(void)
{
    ...
    if ((t - NetArpWaitTimerStart) > ARP_TIMEOUT * CFG_HZ*100) {
        NetArpWaitTry++;
...
restart:
    ...
    for (;;) {
        ...
        if (timeHandler && ((get_timer(0) - timeStart) > timeDelta)) {
            thand_f *x;
            /*
			printf("time elaps %d, timeDelta = %d\n",(get_timer(0) - timeStart), timeDelta);
			*/
...
int PingReply(uchar * pkt_src)
{
    ...
    static uchar mac[6];
    static uchar mac_dest[6];

    volatile IP_t *ip;
    volatile IPaddr_t ipsrc;
    ...
    memcpy(mac_dest,((Ethernet_t *)pkt_src)->et_src, 6);

    pkt = pkt_src;

    pkt += NetSetEther(pkt, mac, PROT_IP);          /* source mac and protocal */
    memcpy(((Ethernet_t *)pkt_src)->et_dest, mac_dest, 6);   /* dest mac */
     ...
     */
     ...
     /*  IP */
    icmph=(struct icmphdr*)(pkt_src+14+20);
    ip = (volatile IP_t *)(pkt_src+14);

    NetCopyIP((void *)&ipsrc,(void *)&ip->ip_src);
    ...
    NetCopyIP((void*)&ip->ip_dst, &ipsrc);      /* - "" - */

    ip->ip_sum   = ~NetCksum((uchar *)ip, IP_HDR_SIZE_NO_UDP / 2);

    /*  ICMP */
    s = &ip->udp_src;               /* XXX ICMP starts here */
    s[0] = htons(0x0000);           /* echo-request, code */
    s[1] = 0;                       /* checksum */
    //s[2] = 0;                         /* identifier */
    //s[3] = htons(PingSeqNo++);    /* sequence number */

    s[1] = ~NetCksum((uchar *)s, (htons(ip->ip_len) - 20)/2);

    (void) eth_send(pkt_src, htons(ip ->ip_len)+ 14); /* 14=mac header */
    //PauseSend_macok(pkt_src);
...
int PauseSend_macok(uchar * pkt_src)
{
    struct icmphdr_WX {
         unsigned char  type;
         unsigned char  code;
    ...
    *((u16 *)((char *)pkt+12))=htons(0x8808);           /* source mac and protocal */
    /*
     *  Construct an IP and ICMP header.  (need to set no fragment bit - XXX)
     */
    ...
    *((u16 *)ip) = htons(0x0001);                           /* IP_HDR_SIZE / 4 (not including UDP) */
    *((u16 *)((char *)ip+2)) = htons(0xFFFF);

    (void) eth_send(pkt_src, 4 + 14); /* 14=mac header */
...
int PauseSend(uchar * pkt_src)
{
    ...
    pkt += NetSetEther(pkt, mac, 0x8808);           /* source mac and protocal */
    memcpy(((Ethernet_t *)pkt_src)->et_dest, mac_dest, 6);   /* dest mac */
...
{
    extern void debug_pck_Rcv_force(unsigned char *pck,int len,unsigned short bd_id);
    Ethernet_t *et;
    IP_t    *ip;
    ARP_t   *arp;
    IPaddr_t tmp;
    int x;
    uchar *pkt;
    #define debug_net(x) //puts("^^^^^^^^^^^^^  ");puts(x)
    ...
    {
        /* supported icmp now in polling mode */
        if (!((ip->ip_p == IPPROTO_ICMP)  || (x==PROT_ARP) || (x==ARPOP_REQUEST))) return;
    }
...
#ifdef ET_DEBUG
            printf("------------ntohs(arp->ar_hrd) =%x != ARP_ETHER=%x\n",ntohs(arp->ar_hrd) ,ARP_ETHER);
#endif
            return;
        }
        if (ntohs(arp->ar_pro) != PROT_IP) {
#ifdef ET_DEBUG
            printf("------------ntohs(arp->ar_pro) =%x != PROT_IP=%x\n",ntohs(arp->ar_hrd) ,PROT_IP);
#endif
            return;
        }
        if (arp->ar_hln != 6) {
#ifdef ET_DEBUG
            printf("------------arp->ar_hln=%x!=6\n",arp->ar_hln);
#endif
            return;
        }
        if (arp->ar_pln != 4) {
#ifdef ET_DEBUG
            printf("------------arp->ar_pln =%x!=4\n",arp->ar_pln );
#endif
            return;
        }
...
#ifdef ET_DEBUG
            printf("------------NetReadIP(&arp->ar_data[16])=%x != NetOurIP=%x\n",NetReadIP(&arp->ar_data[16]) ,NetOurIP);
            print_IPaddr(NetOurIP);
            print_IPaddr(NetReadIP(&arp->ar_data[16]));
#endif
            return;
        }
...
#endif
            debug_net("Got ARP REQUEST, return our IP\n");
            #ifdef CONFIG_NET_MULTI
                memcpy (NetOurEther, eth_get_dev()->enetaddr, 6);
            #else
                memcpy (NetOurEther, bd->bi_enetaddr, 6);
    ...
    case PROT_IP:
        ...
        if (len < IP_HDR_SIZE) {
            ...
            debug_pck_Rcv_force((unsigned char *)inpkt, len, 0xFFFF);
            return;
        }
        if (len < ntohs(ip->ip_len)) {
            printf("total len bad %d < iplen %d\n", len, ntohs(ip->ip_len));
            debug_pck_Rcv_force((unsigned char *)inpkt, len, 0xFFFF);
            return;
        }
        len = ntohs(ip->ip_len);
