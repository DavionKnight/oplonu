...
do_version (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    ...
    printf("------------------------- release note ----------------------------\n");
    printf("v1.5 : support ONU MPW.\n");
    printf("v1.4 : support ONU FPFA.\n");
    printf("v1.2 : remove the bug that command tftp cann't download file from LINUX tftp server more than one times.\n");

#ifdef CONFIG_IPMUX_A
    printf("v1.1 : calculate the average delay value for DDR controller.\n");
#endif
    printf("-------------------------------------------------------------------\n");
...
int do_help (cmd_tbl_t * cmdtp, int flag, int argc, char *argv[])
{
    ...
    for (i = 1; i < argc; ++i) {
        if ((cmdtp = find_cmd (argv[i])) != NULL) {
            ...
            printf ("do_help:Unknown command '%s' - try 'help'"
