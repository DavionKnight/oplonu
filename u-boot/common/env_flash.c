...
static env_t *flash_addr = (env_t *)CFG_ENV_ADDR_INIT;
#endif

#else /* ! ENV_IS_EMBEDDED */

//env_t *env_ptr = (env_t *)CFG_ENV_ADDR;
env_t *env_ptr = (env_t *)(0xbfc00000+ENV_OFFSET);/* CFG_ENV_ADDR_INIT;*//*zw change*/
#ifdef CMD_SAVEENV
//static env_t *flash_addr = (env_t *)CFG_ENV_ADDR;
//static env_t *flash_addr = (env_t *)CFG_ENV_ADDR_INIT;
static env_t *flash_addr = (env_t *)(0xbfc00000+ENV_OFFSET);
#endif
...
void changePtrback()
{
    flash_addr =(env_t *)(0xbfc00000+ENV_OFFSET);
}
...
int saveenv(void)
{
    ...
    dbgFlash ("Protect off %08lX ... %08lX\n",
        (ulong)flash_addr, end_addr);

    if (flash_sect_protect (0, (ulong)flash_addr, end_addr)) {
        printf("protect off 0x%x~0x%x failed.\n",flash_addr, end_addr);
        goto Done;
    }
    ...
    if (up_data) {
        ...
        dbgFlash ("Data (start 0x%x, len 0x%x) saved at 0x%x\n",
    ...
    if (up_data) { /* restore the rest of sector */
        dbgFlash ("Restoring the rest of data to 0x%x len 0x%x\n",
...
#ifndef CFG_NEW_SPI_FLASH
  #ifndef DEBUGMODE 
    if (crc32(0, env_ptr->data, ENV_SIZE) == env_ptr->crc) {
        gd->env_addr  = (ulong)&(env_ptr->data);
        gd->env_valid = 1;
        //printf("calculate env_ptr 0x%x, OK.\n", env_ptr);
        return(0);
    }
  #endif
#else
...
int saveenv(void)
{
    ...
    len  = CFG_ENV_SIZE;
    //printf("flash_sect_addr:%x\n",flash_sect_addr);

    end_addr = flash_sect_addr + len - 1;

    puts ("Erasing Flash...\n");
    //  printf("flash_sect_addr:%x,end_addr:%x\n",flash_sect_addr,end_addr);
    //flash_erase(NULL,3,3);
    #ifdef CONFIG_IPMUX_ATHEROS
    //puts ("Erasing from offset 0x40000\n");
//        #ifdef CONFIG_IPMUX_ONU_1_1_F23
            spi_erase_one_sector(ENV_OFFSET/0x10000);/*kxu 20110408*/
//        #else
//            spi_erase_one_sector(0x40000);//erase spi flash independent
//        #endif
    
    #else
 //   flash_sect_erase(0xb0040000,0xb004ffff);
       spi_erase_one_sector(ENV_OFFSET/0x10000);/*kxu 20110302*/

    #endif
    udelay(1000);
    puts ("\nWriting to Flash... ");
    //printf("env_buffer:%s,flash_sect_addr:%x,len:%d\n",env_buffer,flash_sect_addr,len);
    write_buff(0, env_buffer, flash_sect_addr, len);
    printf("done\n");
    return rcode;
}
#else 
...
#endif /*CFG_NEW_SPI_FLASH  || CFG_SPI_FLASH*/
#endif /* CMD_SAVEENV */
...
void env_relocate_spec (void)
{
    ...
    memcpy (env_ptr, (void*)flash_addr, CFG_ENV_SIZE);/*flash_addr*/
