    ...
    *s_count = 0;
    ...
    for (bank=0,info=&flash_info[0];
        ...
        if (s_first[bank] >= 0) {
            ...
            printf ("Error: start address 0x%x[0x%x, 0x%x] not on sector boundary\n", addr_first, info->start[0], b_end);
            rcode = 1;
            break;
...
{
    flash_info_t *info;
    unsigned long addr_first,addr_last;
    int rcode = 0;
    ...
    //printf("rcode:%d,planned:%d\n",rcode,planned);
    if (planned && (rcode == 0)) {
        //printf("src:%d,count:%d\n",(start-0xbfc00000)/0x10000,(end-0xbfc00000)/0x10000);
        flash_erase(info,(addr_first-0xb0000000)/0x10000,(addr_last-0xb0000000)/0x10000);
...
int do_protect (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    ...
    int rcode = 0;
    reCfgCs0(flash_info[0].size); /*zw add 922*/
    if (argc < 3) {
        printf ("Usage:\n%s\n", cmdtp->usage);
...
U_BOOT_CMD(
    ...
    "start end\n"
    "    - erase SPI flash from addr 'start' to addr 'end'\n"
);
#else
U_BOOT_CMD(
    erase,   3,   1,  do_flerase,
