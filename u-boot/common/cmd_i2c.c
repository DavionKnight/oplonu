...
#if  1// (CONFIG_COMMANDS & CFG_CMD_I2C)
...
// printf("test I2c bus:\n");
 // ipmuxI2CInit(1,0,0,7);
  printf("addr=0x%x, len=0x%x:\n",addr,len);
//  *(unsigned char*)(0xb1000008) = 0x3;
  udelay(1000);
 // i2cRead(0x51, 1, read_data); 
//  i2cRead(addr, len, read_data); 
  drvI2cDevRd(4,0,16,&read_data);  // dev 0x54
  printf("read back data = 0x%x\n",read_data);
 //*(unsigned char*)(0xb1000008) = 0x0;
  return 0;
...
//  *(unsigned char*)(0xb1000008) = 0x3;
  udelay(1000);
 // i2cRead(0x51, 1, read_data); 
 // i2cWrite(addr, len, read_data); 
 // printf("read back data = 0x%x\n",read_data[0]);
 //*(unsigned char*)(0xb1000008) = 0x0;

 drvI2cDevWr(4,addr,len,&val);//dev 0x54
...
int do_i2c_init( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
  char i2cMode,intMode,slvAddr,nRate;

  i2cMode = simple_strtoul(argv[1], NULL, 16);
  intMode = simple_strtoul(argv[2], NULL, 16);
  slvAddr = simple_strtoul(argv[3], NULL, 16);
  nRate = simple_strtoul(argv[4], NULL, 16);

  ipmuxI2CInit ( i2cMode,  intMode,  slvAddr,  nRate);

  return 0;
}

U_BOOT_CMD(
	i2c_read,	3,	1,	do_i2c_read,
	"i2c_read  -   addr len\n",
	"\n    -read i2c device\n"
);
U_BOOT_CMD(
	i2c_write,	4,	1,	do_i2c_write,
	"i2c_write  -   addr len val\n",
	"\n    -write i2c device\n"
);

U_BOOT_CMD(
	i2c_init,	5,	1,	do_i2c_init,
	"i2c_init  -  i2cMode,  intMode,  slvAddr,  nRate .  (1,0,0,7)\n",
	"\n    -init i2c controler\n"
);
