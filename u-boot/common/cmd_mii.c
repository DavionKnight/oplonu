...
int do_miir (cmd_tbl_t * cmdtp, int flag, int argc, char *argv[])
{
	if(argc == 1){
		ipmux_MII_dump(IPMUX_MII_PORT_GE0, 1);
	}
	else{
	//To be continued...	
	}
...
U_BOOT_CMD(
	miir,    2,      1,      do_miir,
	"miir     - MII dump registers\n",
	"miir <regid>    - print <reg> \n"
);

int do_miiw (cmd_tbl_t * cmdtp, int flag, int argc, char *argv[])
{
	unsigned char   reg;
	unsigned short  data;
	...
	printf("reg:%d, data:%d\n",reg,data);
	ipmux_MII_write(0,1,reg,data);
	//ipmux_MII_write (0, 1, 0x10, 0x3c0);
}

U_BOOT_CMD(
	miiw,    3,      1,      do_miiw,
	"miiw     - write registers\n",
	"miiw <regid>    <value> \n"
);
