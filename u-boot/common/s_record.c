...
int srec_decode (char *input, int *count, ulong *addr, char *data)
{
	...
	unsigned char chksum;			/* buffer for checksum	*/
        ...
        /*char tmp[500];         wx:add for debug*/
        /*
        icount++;
        strcpy_word(tmp,input);
        printf("debug:line %d input=[%s] addr=[%x] data=[%s]\n",icount,input,*addr,data);
        */
	...
	if ((*count = hex2_bin(input)) < 0) {
                printf("debug:follow S are not two number ! \n");
