...
#if defined(CONFIG_IPMUX_E_MPW) || defined(CONFIG_IPMUX_E2)
int do_mem_down_ddr (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    u32 i=0;
    u32 k=0;
    int count = 0;
    volatile u32 ddrtemp;
    
    printf("\n Writing DRAM1 bigins:\n");
    
    for(i=0;i<4194304;i+=100)
    {
        *(volatile UINT32 *)0xbf004990 = 0x1;
        *(volatile UINT32 *)0xbf004998 = i;
        *(volatile UINT32 *)0xbf00499c = i;
        while (*(volatile UINT32 *)0xbf004994 & 0x1) {  
        }
        *((volatile UINT32 *)(0xbf004990)) = 0;
    ...
    printf("\nChecking DRAM1 bigins:\n");
    k=0;

    for(i=0;i<4194304;i+=100)
    {   
        *(volatile UINT32 *)0xbf004990 = 0x2;
        *(volatile UINT32 *)0xbf0049a4 = i;
        while (*(volatile UINT32 *)0xbf0049a0 & 0x1) {
           }
        ddrtemp = *((volatile UINT32 *)(0xbf0049a8));
        *((volatile UINT32 *)(0xbf004990)) = 0;
        
        if( i != ddrtemp)
        {
        printf("\nThere was an error taking palce during DRAM1 test\n");
        count++;
        }
        else
        {
    ...
    printf("\nDRAM1 post has finished, there are in %d error total.\n",count);
}
#endif
...
#if defined(CONFIG_IPMUX_E_MPW) || defined(CONFIG_IPMUX_E2)
U_BOOT_CMD(
    dwddr,     1,     1,      do_mem_down_ddr,
    "dwddr    - down ddr test display\n",
    "[.b, .w, .l] address step [# of objects]\n    - down ddr test display\n"
);
#endif
