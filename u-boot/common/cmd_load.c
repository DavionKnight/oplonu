...
//#define debugLoad(a) puts(a)
#define debugLoad(a) 
#if (CONFIG_COMMANDS & CFG_CMD_LOADS)
static ulong load_serial (ulong offset);
static char lastget[0x100];
...
int do_load_serial (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	load_baudrate = current_baudrate = gd->baudrate;
        printf("baudrate=%d",load_baudrate);
#endif
	...
	puts ("## Ready for S-Record download ...\n");

	addr = load_serial (offset);
    //printf("##debug:after load_serial,addr=%x\n",addr);
	...
	if (addr == ~0) {
		puts ("## S-Record download aborted\n");
		rcode = 1;
	} else {
...
load_serial (ulong offset)
{
	...
	ulong	end_addr   =  0;
	int	line_count =  0;
	debugLoad("in load_serial\n");
	while (read_record(record, SREC_MAXRECLEN + 1) >= 0) {
		type = srec_decode (record, &binlen, &addr, binbuf);
		
		if (type < 0) {
                        puts("Invalid s-rec type:");
			printf(" %x\n",type);	
		...
		if (!do_echo) {	/* print a '.' every 100 lines */
			...
			{
				putc ('.');
				//printf("lc=%d ",line_count);
			}
				//putc ('.');
		}
	}
        
        printf("debug:while statement error \n");
