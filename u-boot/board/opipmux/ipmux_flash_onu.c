...
/*
#ifdef FPGA_IPMUX_OPULAN
#define DEBUG_FLASH
*/
#ifdef FPGA_IPMUX_ONU_8BIT
...
unsigned long flash_init (void)
{
	...
	int i;
       ipMuxRegWrite(REG_CS0_BASE_ADDR, (PHYS_FLASH_1&0x1FFFFFFF)>>12);  /*zw 922*/
...
void flash_print_info (flash_info_t *info)
{
	...
	uchar topboottype[] = "top boot sector";
       reCfgCs0(flash_info[0].size); /*zw 922*/
		...
		if(addr != 0xbfc00000){
			...
			*addr = (FPW)0x00300030;	 			 /* erase sector */
			...
			for(i = 0; i< 8; i++){
				...
				*(addr + 0x2000*i) = (FPW)0x00300030;	 			 /* erase sector */
			}
		}
...
static int write_word_amd (flash_info_t *info, FPWV *dest, FPW data)
{
    ...
    if ((*dest & data) != data) {
	printf("wwei debug: *dest = %x, dest =%x, data =%x.\n", *dest, dest, data);
	return (2);
    }
