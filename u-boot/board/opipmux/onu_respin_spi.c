...
#include "asm-mips/types.h"
 ...
 #if defined(DRV_NEW_SPI)||defined(CFG_NEW_SPI_FLASH)
...
{
    OPL_STATUS retVal,tmp;
//    printf("spiTransLenSet W len=0x%x\n",len);
/*kxu :  delay->write len reg->delay->read len reg*/
  //  delay(1000);  
    retVal = RegFieldWrite(REG_SPI_LEN, 0, 16, len );
//        printf("spiTransLenSet R len=0x%x\n",*(unsigned*)0xbf000c44);
  //  delay(1000);
    //tmp=*(unsigned*)0xbf000c44;
 //   tmp=*(volatile __u32 *)(regBaseAddr+ REG_SPI_LEN*4);
...
//    		printf("len1=0x%x\n\n",len);
	...
	switch (((len>>8)-1)/32){//  0x8080
		...
		case 3:
			...
			break;
		default:
			printf("Error: invalid length=0x%x...\n",len);
		}
#endif
	return retVal;
}
...
//	OPL_STATUS regVal = 0;
	...
	__u32 ready_buf_id = 0;
       ...
       /*be better to do it before each spi operating */
  	retVal = waitBusy();
	...
	PL
	/*
	printf("0xbf000c20  = 0x%x\n",*(unsigned*)0xbf000c20);
	printf("0xbf000c24  = 0x%x\n",*(unsigned*)0xbf000c24);
	printf("0xbf000c28  = 0x%x\n",*(unsigned*)0xbf000c28);
	printf("0xbf000c2c  = 0x%x\n",*(unsigned*)0xbf000c2c);
	printf("0xbf000c30  = 0x%x\n",*(unsigned*)0xbf000c30);
	printf("0xbf000c34  = 0x%x\n",*(unsigned*)0xbf000c34);
	printf("0xbf000c38  = 0x%x\n",*(unsigned*)0xbf000c38);
	printf("0xbf000c3c  = 0x%x\n",*(unsigned*)0xbf000c3c);
*/
...
//	printf("read spi flash status  = 0x%x\n",tmp_data.data0);
	return retVal;
...
//	OPL_STATUS regVal = 0;
 ...
 //   		printf("write en reg len=0x%x\n",*(unsigned int*)0xbf000c44);
	/*get spi valid len */
	retVal = spiTransLenGet(&len);
//    		printf("write en len=0x%x\n",len);
	/*set cmd for spi flash*/
	(&cmd)->data0 =0x06 ;
	(&cmd)->data1 =0 ;
	(&cmd)->data2 =0 ;
...
//	OPL_STATUS regVal = 0;
	...
	printf("\n chu	cuo  la......... : write spi flash timeout\n");
	return OPL_ERROR;
	OUT:
//		OPL_PRINTF("done\n");
...
/**
*@par Description
* This function read one word to spi flash .
*
* @param [in] *value
*	   pointer  of the word to be programed in flash.
* @param [in] addr_ofs
...
//	printf("*value = 0x%x\n",*value);
	__u32 tmp_data;
	tmp_data = *value;
//	printf("write flash data : 0x%x\n",(__u8)(tmp_data>>24));	
//	printf("write flash data : 0x%x\n",(__u8)(tmp_data>>16));	
//	printf("write flash data : 0x%x\n",(__u8)(tmp_data>>8));	
//	printf("write flash data : 0x%x\n",(__u8)(tmp_data));	

	retVal = spiByteProgram((__u8)(tmp_data>>24),addr_ofs);
	retVal = spiByteProgram((__u8)(tmp_data>>16),addr_ofs+1);
	retVal = spiByteProgram((__u8)(tmp_data>>8),addr_ofs+2);
	retVal = spiByteProgram((__u8)(tmp_data),addr_ofs+3);
	return retVal;
	#endif
...
//	printf("write flash data : 0x%x\n",(__u8)(tmp_data>>8));	
//	printf("write flash data : 0x%x\n",(__u8)(tmp_data));	
//       retVal = spiWordProgram(&tmp_data,addr_ofs);
	#if 1
	retVal = spiByteProgram((__u8)(tmp_data>>24),addr_ofs);
	retVal = spiByteProgram((__u8)(tmp_data>>16),addr_ofs+1);
	retVal = spiByteProgram((__u8)(tmp_data>>8),addr_ofs+2);
...
OPL_STATUS sstWordProgram(__u32 *value,__u32 addr_ofs)
{
	...
	retVal = spiByteProgram((unsigned char)((*value&(0xff000000))>>24),addr_ofs);
	retVal = spiByteProgram((unsigned char)((*value&(0xff0000))>>16),addr_ofs+1);
	retVal = spiByteProgram((unsigned char)((*value&(0xff00))>>8),addr_ofs+2);
	retVal = spiByteProgram((unsigned char)(*value&(0xff)),addr_ofs+3);
	return retVal;
}
#endif
#if 0
...
{
    __u32 value=0;
    spiWordProgram(&value, addr_ofs);
//    printf("addr_ofs:0x%x, value:0x%0x",addr_ofs,value);
    return  (value);
}
#endif
...
//		 printf("u-boot write flash...\n");
		...
		/*wangxin:big endian*/
			...
			for (i = 0; i < sizeof(data); i++) {
				...
				{	
					data += *((uchar *)addr + i);//This place will be caused spi host read that spi reg will be changed at moment.
...
//			}
//			printf("###ofs=%x data=%x\n", ofs,data );
//			switch(info->flash_id>>24){
//				printf("\ndata = 0x%x\n",data);
//			   case FLASH_MAN_SST :
                 sst_write_one_word(&data,ofs);
 //               spiByteProgram((__u8*)(&data),ofs);
//				 break;
...
//		__u32 header1 = 0x030b0108 ;  /**fast read**/
           //__u32 header1 = 0x03030108; /*normal read*/
 //          __u32 header1 = 0x03BB0108; /*: 2x I/O Read Mode*/
// 	__u32 header1 = 0x030b0101 ;  /**fast read**/  //can't use it 
#ifdef  ONU1_1_SPI_REG
        __u32 header1 = 0x01030200; /*normal read*/
#else
        __u32 header1 = 0x030b0108 ;  /**fast read**/
#endif
...
//              	printf("ofs = %d  \n",ofs);//dbg
			...
			/*####################################*/
			if (ofs == 0){
//				printf("header programing : info->flash_id>>24: 0x%x\n",info->flash_id>>24);
//				switch(info->flash_id>>24){
//				   case FLASH_MAN_SST :
		#if 0			
					    spiByteProgram(cc1,0x0);
					    spiByteProgram(cc2,0x1);
					    spiByteProgram(cc3,0x2);
					    spiByteProgram(cc4,0x3);
					    spiByteProgram(cc5,0x4);
						
						spiByteProgram(&cc1,0x40);
					    spiByteProgram(&cc2,0x41);
					    spiByteProgram(&cc3,0x42);
					    spiByteProgram(&cc4,0x43);
					    spiByteProgram(&cc5,0x44);
	#endif 
#if 1						
                    //    break;
			//	   default :
