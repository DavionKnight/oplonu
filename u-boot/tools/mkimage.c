		...
		while (*++*argv) {
			...
			case 'n':
				...
				goto NXTARG;
			     ...
			     if (--argc<=0)
                                  usage ();
			       chipId = strtoul (*++argv, (char **)&ptr, 16);			
				goto NXTARG;	
			     goto NXTARG;
                   case 'r':
			     if (--argc<=0)
                                  usage ();
			     revnum=strtoul (*++argv, (char **)&ptr, 16);	
			     goto NXTARG;
                   case 'f':
			     if (--argc<=0)
                                  usage ();
...
#ifdef OPULAN_IMAGE_HEADER	
	hdr->ih_chipid = chipId;
	hdr->ih_revnum = revnum;
	hdr->ih_flags = flags;
#endif
...
#ifdef OPULAN_IMAGE_HEADER		
			 "          -i  ==> set chip ID of Opulan\n"
			 "          -r  ==> set SVN revsion number\n"
			 "          -f  ==> set options of files\n"
 #endif
		);
	exit (EXIT_FAILURE);
}
