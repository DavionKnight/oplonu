 ...
 *
 *  $Id: //depot/sw/abu/opconn/release/v3.2.1/os/linux-2.6.21.5/drivers/serial/8250.c#1 $
 *
...
#define EXAR_PORT3_BASE  EXAR_PORT_BASE+0x00
#define EXAR_PORT4_BASE  EXAR_PORT_BASE+0x08
#define EXAR_PORT_SIZE 0x7
#define EXAR_CLK 14745600
#define OPL_BASE  0xbf000000
#define OPL_CS1_BASE 0x3024
#define OPL_CS2_BASE 0x3028
#define OPL_CS1_MASK 0x304C
#define OPL_CS2_MASK 0x3050


#define EXAR_PHY2_ADDR 0x1f900
#define EXAR_PHY1_ADDR 0x1f800
...
static struct plat_serial8250_port exar_data[] = {
	PORT(EXAR_PORT1_BASE,OPL_EXAR_PORT_IRQ),
	PORT(EXAR_PORT2_BASE,OPL_EXAR_PORT_IRQ),
	PORT(EXAR_PORT3_BASE,OPL_EXAR_PORT_IRQ),
	PORT(EXAR_PORT4_BASE,OPL_EXAR_PORT_IRQ),
	{0},
};

static struct resource exar_resource[]={
	[0]={
		.start =EXAR_PORT1_BASE,
		.end=EXAR_PORT1_BASE+EXAR_PORT_SIZE,
		.flags=IORESOURCE_MEM,
		},
   	[1]={
		.start =EXAR_PORT2_BASE,
		.end=EXAR_PORT2_BASE+EXAR_PORT_SIZE,
		.flags=IORESOURCE_MEM,
   		},
   	[2]={
		.start =EXAR_PORT3_BASE,
		.end=EXAR_PORT3_BASE+EXAR_PORT_SIZE,
		.flags=IORESOURCE_MEM,
		},
   	[3]={
		.start =EXAR_PORT4_BASE,
		.end=EXAR_PORT4_BASE+EXAR_PORT_SIZE,
		.flags=IORESOURCE_MEM,
		},
   	[4]={
		.start =OPL_EXAR_PORT_IRQ,
		.end=OPL_EXAR_PORT_IRQ,
...
static struct platform_device exar_device = {
	...
	.num_resources = ARRAY_SIZE(exar_resource),
	.resource = exar_resource,
	.dev			= {
...
static int serial8250_request_std_resource(struct uart_8250_port *up)
{
	...
	case UPIO_MEM:
		...
		if (!request_mem_region(up->port.mapbase, size, "exar_serial")) {
			ret = -EBUSY;
			break;
		}
	...
	case UPIO_HUB6:
	case UPIO_PORT:
		if (!request_region(up->port.iobase, size, "exar_serial"))
			ret = -EBUSY;
		break;
	}
	return ret;
...
static int __init serial8250_init(void)
{
	...
	serial8250_isa_devs = platform_device_alloc("exar8250",
						    PLAT8250_DEV_EXAR_ST16C554);
	if (!serial8250_isa_devs) {
		ret = -ENOMEM;
		goto unreg_uart_drv;
	}
    #define GPIO_DIR 0xb21
	...
	ugpio =readl((u32 *)(OPL_BASE+GPIO_MODE_SEL *4)) |(1<<11);
	writel(ugpio,(unsigned int *)(OPL_BASE+GPIO_MODE_SEL *4));

	ugpio =readl((u32 *)(OPL_BASE+GPIO_LEVEL_MODE *4)) |(~(1<<11));
	writel(ugpio,(unsigned int *)(OPL_BASE+GPIO_LEVEL_MODE *4));
	
	/* set the device mode to default 232 mode */
