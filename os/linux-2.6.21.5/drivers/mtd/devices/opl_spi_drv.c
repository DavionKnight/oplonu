...
* 
* @file opl_spi_drv.c :Driver source file for SPI module
*
* DESCRIPTION: 
*
* Date Created:
 ...
 */
static struct flash_info __devinitdata opl_spi_data [] = {
...
#define ONU_OPCONN		0x63511
#define ONU_RESPIN		0x63512
#define IS_ONU_OPCONN (((*(volatile u32 *)(0xBF000000))&0xFFFFF) == ONU_OPCONN)
...
int spi_erase_one_sector(unsigned long ofs, unsigned char erasecode)
{
	...
	else
	{
		...
		if (buf_id)
		{
			p_spi->tr4 = (erasecode<<24 & 0xff000000) | ofs; /* set transmit register */
		}
		else
		{
			p_spi->tr0 = (erasecode<<24 & 0xff000000) | ofs; /* set transmit register */
...
void spi_write_status(void)
{
	...
	else
	{
		...
		if (buf_id)
		{
			p_spi->tr4 = 0x0100; /* set transmit register */
		}
		else
		{
			p_spi->tr0 = 0x0100; /* set transmit register */
...
void spi_write_one_byte(unsigned long ofs, unsigned char value)
{
	...
	else
	{
		...
		if (buf_id)
		{
			p_spi->tr5 = 0x02; /* set transmit register */
			p_spi->tr4 = (ofs << 8) | value; /* set transmit register */
		}
		else
		{
			p_spi->tr1 = 0x02; /* set transmit register */
		...
		u8 *rxbuf, unsigned n_rx)
{
	...
	else
	{
		...
		if (buf_id)
		{
			...
			p_spi->tr4 = 0x0; /* set transmit register */
		}
		else
		{
			p_spi->tr1 = 0x9f; /* set transmit register */
...
static struct flash_info *__devinit jedec_probe(struct spi_device *spi)
{
	...
	for (tmp = 0, info = opl_spi_data;
			tmp < ARRAY_SIZE(opl_spi_data);
			tmp++, info++) {
...
static int __init opconn_probe(struct platform_device *plat_dev)
{
	...
	if (data && data->type) {
		...
		for (i = 0, info = opl_spi_data;
				i < ARRAY_SIZE(opl_spi_data);
				i++, info++) {
	...
	if (mtd_has_partitions()) {
		...
		printk(&spi->dev, "ignoring %u default partitions on %s\n",
...
int opl_spi_drv_init(void)
{
	return platform_driver_register(&opconn_spidrv);
}

static void opl_spi_drv_exit(void)
{
	platform_driver_unregister(&opconn_spidrv);
}

module_init(opl_spi_drv_init);
module_exit(opl_spi_drv_exit);
