...
spinlock_t dma0txlock;
spinlock_t dma1txlock;
...
void print_mem(unsigned char *p, u32 len, u8 flg)
{
  u32 pflg, i;
...
static void inline read_tx_bd(struct ipmux_fe_priv *fep, u32 index)
{
  u32 i = 0;
  u32 ctrl = 0;
...
static void inline write_tx_bd(struct ipmux_fe_priv *fep, u32 index)
{
  u32 i = 0;
  u32 ctrl = 0;
...
static void inline read_rx_bd(struct ipmux_fe_priv *fep, u32 index)
{
  u32 i = 0;
  u32 ctrl = 0;
...
static void inline write_rx_bd(struct ipmux_fe_priv *fep, u32 index)
{
  u32 i = 0;
  u32 ctrl = 0;
...
#ifdef CONFIG_ONU_FE_UT_F23_ETH
...
int onu_start_xmit_buf(struct net_device *ndev, void *skb, u32 len, u8 flg)
{
  unsigned short stats_pos = 0;
 ...
 // unsigned char *printbuf;
  ...
  {
    devtmp = lanndev;
    //spin_lock(&dma1txlock);
  }
  else
  {
    devtmp = wanndev;
   // spin_lock(&dma0txlock);
  }
#else
  devtmp = lanndev;
  //spin_lock(&dma0txlock);
  ...
  while (fep->tx_buffers[index].bd_info & BD_VALID_BIT)
  {
    ...
    {
       //spin_lock(&fep->tx_lock);
       fep->tx_head = ((fep->tx_head-1+MAX_TX_DESC)%MAX_TX_DESC);
	   //spin_unlock(&fep->tx_lock);
      printk("dma not ready for send=%d, tx head %d tail %d netif_queue_stopped %d.\n", index, fep->tx_head, fep->tx_tail, netif_queue_stopped(fep->ndev));
      goto fail;
    }
  }
  /*
  ...
  {
     netif_stop_queue(devtmp);
	 goto fail;
  }*/

  if (fep->tx_buffers[index].skb != NULL){
  	dev_kfree_skb_irq(fep->tx_buffers[index].skb);
  	fep->tx_buffers[index].skb = NULL;
  }

  //fep->tx_tail = ((index+1)%MAX_TX_DESC);
  ...
  dprintk_tx("dev_id=%d, index=%d, buf=%x, length=%d\n", dev_id, index, buf, lengthLocal);
	
#ifdef CONFIG_ONU_FE_UT_F23_ETH
  if (dev_id < MAX_DMA1_INTERFACE)
  {
  #if 0  //use broadcast to send
    if (MACEQUAL(auc_broadcast_mac, sbuf))
    {
  ...
  if ((gSysVoiceVlan[dev_id].VoipTaggedValid == 1) && (flg == 0))
#else
  if (flg == 0)
  {
  ...
  if ((gSysVoiceVlan[dev_id].VoipTaggedValid == 1) && (dev_id != ETH0_INTERFACE_ID) && (flg == 0))
#endif
  {
 	if (dev_id >= MAX_DMA1_INTERFACE)	
	{
  ...
  memcpy(buf, ((u8 *)sbuf)+12, len-12);
	
  print_mem(sbuf, len, 0);
  status = 0;
  status |= ( (lengthLocal<< BD_LENGTH_SHIFT ) | BD_VALID_BIT);
  ...
  else
  {
    ...
    fep->stats[stats_pos].tx_bytes += lengthLocal;
  }

  fep->tx_buffers[index].length = 0;

 /*fep->tx_head = ((fep->tx_head+1)%MAX_TX_DESC);*/
 dprintk_tx("tx send lengthLocal=%x\n",lengthLocal);
 
 print_mem(buf-12, lengthLocal, 0);
 ...
 // spin_unlock(&fep->tx_lock);
  ...
  if (dev_id < MAX_DMA1_INTERFACE)
  {
    spin_unlock(&dma1txlock);
  }
  else
  {
    spin_unlock(&dma0txlock);
  }
#else
  spin_unlock(&dma0txlock);
#endif
...
static int onu_start_xmit(struct sk_buff *skb, struct net_device *ndev)
{
 ...
 // unsigned char *printbuf;
  unsigned long lengthLocal;  
  u32  i = 0, status= 0,flg = 0;  
  u32 head_tmp;
  u32 tail_tmp;  
  struct ipmux_fe_priv *fep;
  struct net_device *devtmp;
  //printk("wfxu ********************onu_start_xmit line:%s %d\n ",__FUNCTION__,__LINE__);
  ...
  len = skb->len;*/
   ...
   return onu_start_xmit_buf(ndev, skb, skb->len, 0);
...
static void onu_tx_interrupt(struct ipmux_fe_priv *fep)
{
  ...
  {
    if (fep->tx_buffers[tx_tail].skb != NULL)
    {
...
void dma_rx_receive_kernel(int dmaflg, void *dma_buf, int pkt_len, struct net_device *devtmp, int with_vlan, struct ipmux_fe_priv *fep)
{
  ...
  skb_reserve(skb,2);
  skb->dev = devtmp;
  print_mem(dma_buf, pkt_len, dmaflg); 

  dprintk_rx("packetLength=%d\n", pkt_len);
  ...
  if (with_vlan)
  {
    if (0 == dmaflg)
    {
      ((char *)dma_buf)[19] = ((char *)dma_buf)[15];
      ((char *)dma_buf)[18] = ((char *)dma_buf)[14];
      ((char *)dma_buf)[17] = ((char *)dma_buf)[13];
      ((char *)dma_buf)[16] = ((char *)dma_buf)[12];
      ((char *)dma_buf)[15] = ((char *)dma_buf)[11];
      ((char *)dma_buf)[14] = ((char *)dma_buf)[10];
      ((char *)dma_buf)[13] = ((char *)dma_buf)[9];
      ((char *)dma_buf)[12] = ((char *)dma_buf)[8];
      ((char *)dma_buf)[11] = ((char *)dma_buf)[7];
      ((char *)dma_buf)[10] = ((char *)dma_buf)[6];
      ((char *)dma_buf)[9] = ((char *)dma_buf)[5];
      ((char *)dma_buf)[8] = ((char *)dma_buf)[4];

      memcpy(skb_put(skb,pkt_len-8), (u8 *)(dma_buf+8), pkt_len-8);

    }
    else
    {
      ((char *)dma_buf)[17] = ((char *)dma_buf)[11];
      ((char *)dma_buf)[16] = ((char *)dma_buf)[10];
      ((char *)dma_buf)[15] = ((char *)dma_buf)[9];
      ((char *)dma_buf)[14] = ((char *)dma_buf)[8];
      ((char *)dma_buf)[13] = ((char *)dma_buf)[7];
      ((char *)dma_buf)[12] = ((char *)dma_buf)[6];
      ((char *)dma_buf)[11] = ((char *)dma_buf)[5];
      ((char *)dma_buf)[10] = ((char *)dma_buf)[4];
      ((char *)dma_buf)[9] = ((char *)dma_buf)[3];
      ((char *)dma_buf)[8] = ((char *)dma_buf)[2];
      ((char *)dma_buf)[7] = ((char *)dma_buf)[1];
      ((char *)dma_buf)[6] = ((char *)dma_buf)[0];
      
      memcpy(skb_put(skb,pkt_len-6), (u8 *)(dma_buf+6), pkt_len-6);
    }
  }
  else
  {
...
static int dma_rx_receive(struct net_device *ndev, int budget)
{
  ...
  while ((received < budget) && find_valid_rx(received, fep, index, &valid_index))
  {
    ...
    else
    {
      ...
      #endif
        offset0 = 4;/*means the packet comes from where ge or pon*/
        offset1 = 0;
      }
      else
      {
        ...
        offset0 = 0;
        offset1 = 2; /*means 2 bytes atheros 8228 head (u16 usVersionHi:2;u16 usPriority:2;u16 usPackType:4;u16 usFromCpu:1;u16 usPortNum:7;)*/
      }
      ...
      if ((((u8 *)dma_buf)[12+offset0+offset1] == 0x81) && (((u8 *)dma_buf)[13+offset0+offset1] == 0x00))
      {
        ...
        else
        {
          ...
          {
            dprintk_rx("index=%d, valid=%d, vlan_tag=%x\n", i, gSysVoiceVlan[i].VoipTaggedValid, gSysVoiceVlan[i].VoiceVlanId);
            if ((gSysVoiceVlan[i].VoipTaggedValid==1) && (gSysVoiceVlan[i].VoiceVlanId==vlan_tag->bfVoiceVlan))
            {
              devnownum = i;
              break;
            }
      ...
      print_mem(dma_buf+0, pkt_len+offset0, 1);	
      
      if (((char *)dma_buf+offset0)[0] == 0x01)
      {
      ...
      for (i=devendnum-1; i>=devstartnum; i--)
      {
        ...
        if (devtmp)
        {
          if (MACEQUAL((char *)devtmp->dev_addr, (char *)dma_buf+offset0))
          {
            ...
            {
              if (((((u8 *)dma_buf)[34+offset] == 0x00)&&(((u8 *)dma_buf)[35+offset] == 0x43)) || 
                  ((((u8 *)dma_buf)[34+offset] == 0x00)&&(((u8 *)dma_buf)[35+offset] == 0x44)))
              {
                if (ipout != NULL)
                {
                 dprintk_rx("(unicast)dhcp relay packet dma_rx_receive_kernel\n");               
                  //if (ipout->ifa_list != NULL)
                  {
#if 1 /* modified for dhcp relay */
					dma_rx_receive_kernel(dmaflg, dma_buf, pkt_len, devtmp, with_vlan, fep);
#else
                    iptmp = ipout->ifa_list->ifa_address;
                    if (iptmp == 0)
                    {
                      dma_rx_receive_kernel(dmaflg, dma_buf, pkt_len, devtmp, with_vlan, fep);
                      goto done;
              ...
              else
              {
                ...
                if (ipout != NULL)
                {
                  if (ipout->ifa_list != NULL)
                  {
                    ipin = piphead->daddr;
                    iptmp = ipout->ifa_list->ifa_address;
                    masktmp = ipout->ifa_list->ifa_mask;
          ...
          else if (MACEQUAL(auc_broadcast_mac, (char *)dma_buf+offset0))
          {
            ...
            else if ((((u8 *)dma_buf)[12+offset]==0x08) && (((u8 *)dma_buf)[13+offset]==0x00))
            {
              if (((((u8 *)dma_buf)[34+offset] == 0x00)&&(((u8 *)dma_buf)[35+offset] == 0x43)) || 
                  ((((u8 *)dma_buf)[34+offset] == 0x00)&&(((u8 *)dma_buf)[35+offset] == 0x44)))
              {
                dprintk_rx("broadcast dhcp relay packet dma_rx_receive_kernel\n");  
              	if ((((u8 *)dma_buf)[39+offset+sizeof(struct tagDHCP_PKT)] == 0x35) &&  /* 0x35=DHCP_MESSAGE_TYPE */
              		(((u8 *)dma_buf)[39+offset+sizeof(struct tagDHCP_PKT)+1] == 1) && 
              		((((u8 *)dma_buf)[39+offset+sizeof(struct tagDHCP_PKT)+2] == 2) ||  /* 2=DHCPOFFER */
              		(((u8 *)dma_buf)[39+offset+sizeof(struct tagDHCP_PKT)+2] == 5)))    /* 5=DHCPACK */
					{
						dprintk_rx("broadcast dhcp relay package!\n");
						el_appdma_t *p_s_appdma = NULL;
				...
				{
					dprintk_rx("broadcast dhcp packet!\n");               	
					/*struct iphdr* piphead;
					  struct in_device *ipout=devtmp->ip_ptr;
					  if (ipout != NULL)*/
				  {
...
#if 1   /* ghn++ for Eoc automatic discovery on 110310 */         
            else if ((((u8 *)dma_buf)[12+offset]==0x88) && (((u8 *)dma_buf)[13+offset]==0xB7))
            {
                dprintk_rx(" ********* Eoc automatic discovery protocol package!\n");
                el_appdma_t *p_s_appdma = NULL;
          ...
          {
            dprintk_rx("other unusefule dhcp package\n");
            if ((((u8 *)dma_buf)[12+offset]==0x08) && (((u8 *)dma_buf)[13+offset]==0x00))
            {
              if (((((u8 *)dma_buf)[34+offset] == 0x00)&&(((u8 *)dma_buf)[35+offset] == 0x43)) || 
                  ((((u8 *)dma_buf)[34+offset] == 0x00)&&(((u8 *)dma_buf)[35+offset] == 0x44)))
              {
                dprintk_rx("unknown dhcp package\n");
                el_appdma_t *p_s_appdma = NULL;
          #if 1      /* modified for onu-busybox unrcved dhcp-offer from olt */ 
                dprintk_rx("(debug(LINE %d))dev name=%s\n", __LINE__,devtmp->name);         
                dma_rx_receive_kernel(dmaflg, dma_buf, pkt_len, devtmp, with_vlan, fep);
          #else
		        if ((p_s_appdma=getdmabufaddress(dmaflg)) != NULL)
		        {
      ...
      if (skip_cnt == devendnum-devstartnum)
      {
      	el_appdma_t *p_s_appdma = NULL;
        if ((p_s_appdma=getdmabufaddress(dmaflg)) != NULL)
        {
...
{
#ifndef CONFIG_ONU_FE_UT_F23_ETH
  ...
  *(volatile u32*)(0xbf000000+0x909*4) = 0x1;  //1000m for f23,f13, 100m must be 0
  ...
  /* CPDMA policing  */
  *(volatile u32*)0xbf0180cc = 19; //30;
  *(volatile u32*)0xbf0180d0 = 10;
  *(volatile u32*)0xbf0180d4 = 19; //30;
...
int set_ipmux_dma_tx_buf_data(u8 dmanum, char *appbuf, int len)
{
  ...
  onu_start_xmit_buf(ndev, TxBufdma, len, 1);
...
static int __init onu_fe_probe(struct platform_device *pdev)
{
  ...
  DmaBufInit(0);
  DmaBufInit(1);
  spin_lock_init(&dma0txlock);
  spin_lock_init(&dma1txlock);
#else
  DmaBufInit(0);
  spin_lock_init(&dma0txlock);
#endif
