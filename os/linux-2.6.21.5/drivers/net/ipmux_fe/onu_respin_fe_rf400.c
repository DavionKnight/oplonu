...
#define WORKQ_MOD   (1)
//#define CONFIG_ONU_FE_RESPIN_C13_ETH 
//#define CONFIG_ONU_FE_RESPIN_F23_ETH
#ifdef WORKQ_MOD
...
static struct atheros_4bytes_head_t atheros_4bytes_head;
...
static u8 lan_flag = 0;
static u8 atheros_head_flag = 0;
...
#if defined  (CONFIG_ONU_FE_RESPIN_C13_ETH )||defined  (CONFIG_ONU_FE_RESPIN_F23_ETH )
...
#ifdef WORKQ_MOD
	if (((fep->tx_head-fep->tx_tail)+MAX_TX_DESC) % MAX_TX_DESC >= 16)
	{
...
static int onu_start_xmit(struct sk_buff *skb, struct net_device *ndev)
{
  ...
  while (fep->tx_buffers[index].bd_info & BD_VALID_BIT)
  {
    ...
    if (++i > MAX_WAIT_CNT)
    {
      printk("dma not ready for send=%d\n", index);
      goto fail;
    }
  }
  ...
  {    
    ...
    buf += 6;
#ifdef CONFIG_ONU_FE_RESPIN_F23_ETH
  ...
  MEMCPY6B(buf, ((u8 *)sbuf)+6);
  buf += 6;
  #if defined(CONFIG_ONU_FE_RESPIN_RF400_ETH)
    MEMCPY4B(buf, &atheros_4bytes_head);
    buf += 4;
    length += 4;
  #endif
...
static void onu_tx_interrupt(struct ipmux_fe_priv *fep)
{
  ...
  while(fep->tx_tail != fep->tx_head)
  {
    ...
    {
      dev_kfree_skb_irq(fep->tx_buffers[fep->tx_tail].skb);
...
void dma_rx_receive_kernel(int dmaflg, void *dma_buf, int pkt_len, struct net_device *devtmp, int with_vlan, struct ipmux_fe_priv *fep)
{
  struct sk_buff *skb;
  unsigned short stats_pos = 0;
  unsigned char  offset_src, offset_dst;
  unsigned char  qinq_ctpid_offset;
  ...
  dprintk_rx("###############  with vlan ###############\n");
    if (0 == dmaflg)
    {
      offset_src = 4;
      offset_dst = 8;
	  qinq_ctpid_offset = 20;
	  
      #if defined(CONFIG_ONU_FE_RESPIN_RF400_ETH)
        if(1 == lan_flag)
        {
            if(1 == atheros_head_flag)
            {
                offset_src = 4;
                offset_dst = 12;       /* need to cut the 4bytes opconn header + 4bytes vlan tag + 4bytes ar8327 atheros header */
				qinq_ctpid_offset = 24;
            }
        }
      #endif
	  if ((((u8 *)dma_buf)[qinq_ctpid_offset] == 0x81) && 
	  	(((u8 *)dma_buf)[qinq_ctpid_offset+1] == 0x00))
	  {
          offset_dst += 4;
	  }
    }
   ...
   dprintk_rx("$$$$$$$$$$$$$$$$$  no vlan $$$$$$$$$$$$$$$$$ \n");
    if (0 == dmaflg)
    {
      ...
      {  
          ...
          {
              offset_src = 4;
	       offset_dst = 8;       /* need to cut 4bytes opconn header + 4bytes ar8327 atheros header */
          }
      }
      #endif
    }
    else
    {
      ...
      ((char *)dma_buf)[11+offset_dst] = ((char *)dma_buf)[11+offset_src];
      ((char *)dma_buf)[10+offset_dst] = ((char *)dma_buf)[10+offset_src];
      ((char *)dma_buf)[9+offset_dst] = ((char *)dma_buf)[9+offset_src];
      ((char *)dma_buf)[8+offset_dst] = ((char *)dma_buf)[8+offset_src];
      ((char *)dma_buf)[7+offset_dst] = ((char *)dma_buf)[7+offset_src];
      ((char *)dma_buf)[6+offset_dst] = ((char *)dma_buf)[6+offset_src];
      ((char *)dma_buf)[5+offset_dst] = ((char *)dma_buf)[5+offset_src];
      ((char *)dma_buf)[4+offset_dst] = ((char *)dma_buf)[4+offset_src];
      ((char *)dma_buf)[3+offset_dst] = ((char *)dma_buf)[3+offset_src];
      ((char *)dma_buf)[2+offset_dst] = ((char *)dma_buf)[2+offset_src];
      ((char *)dma_buf)[1+offset_dst] = ((char *)dma_buf)[1+offset_src];
      ((char *)dma_buf)[0+offset_dst] = ((char *)dma_buf)[0+offset_src];
      
      memcpy(skb_put(skb,pkt_len-offset_dst), (u8 *)(dma_buf+offset_dst), pkt_len-offset_dst);
...
static int dma_rx_receive(struct net_device *ndev, int budget)
{
  ...
  int pkt_len;
  int rev;
  void *pfe_pkt, *dma_buf, *debug_buf, *atheros_head_type;
  u32 status, valid_index;
  struct ipmux_fe_priv *fep = netdev_priv(ndev);
  struct net_device *devtmp;
  int devstartnum, devendnum, devnownum;
  int offset0, offset1, offset2, offset, ar8327_head_offset;
  int with_vlan, i;
  int dmaflg = 0;
  int skip_cnt = 0;
  ...
  while ((received < budget) && find_valid_rx(received, fep, index, &valid_index))
  {
    ...
    else
    {
	 ...
	 if(1 == lan_flag)
	 {
            if ((((u8 *)dma_buf)[12+offset0+offset1] == 0x81) && (((u8 *)dma_buf)[13+offset0+offset1] == 0x00)&&(((u8 *)dma_buf)[14+offset0+offset1]&0xf == 0x0f)&&(((u8 *)dma_buf)[15+offset0+offset1] == 0xff))
	      {  
	          memcpy((char *)&(dma_buf[12+offset0+offset1]), (char *)&(dma_buf[12+4+offset0+offset1]), pkt_len-4-12);   /* from ar8327, delete the vlan tag with 4095 added on the ge port */
	          pkt_len = pkt_len - 4;
	      	}
		 
	      if ((((u8 *)dma_buf)[12+offset0+offset1] == 0x91) && (((u8 *)dma_buf)[13+offset0+offset1] == 0x00))
	      {  
	          offset1 = 4;      /* rf400-gna need to add ar8327 4 bytes atheros header when atheros header is before vlan tag */
		   atheros_head_flag = 1;
	      	}
	 }
	 #endif
...
dma0_receive_output:	  
      #if defined(CONFIG_ONU_FE_RESPIN_RF400_ETH)
      dprintk_rx("pkt_len=%d, dmaflg=%d, with_vlan=%d, offset0=%d, offset1=%d, offset2=%d, devstartnum=%d, devendnum=%d, devnownum=%d, lanflag=%d, atherosheadflag=%d\n", 
                  pkt_len, dmaflg, with_vlan, offset0, offset1, offset2, devstartnum, devendnum, devnownum,lan_flag, atheros_head_flag);
      #else
...
process_pon_data:
      ...
      for (i=devendnum-1; i>=devstartnum; i--)
      {
        devtmp = onundevs[i];
	 #if defined(CONFIG_ONU_FE_RESPIN_RF400_ETH)
        if ((with_vlan) && (i!=devnownum) && (0xF!=devnownum)&&(i!=ETH0_INTERFACE_ID))
	 #else
...
void el_xmit_workq(struct work_struct *work)
{
  ...
  while(feptmp->tx_tail != feptmp->tx_head)
  {
    ...
    {
      dev_kfree_skb(feptmp->tx_buffers[feptmp->tx_tail].skb);
...
#ifdef CONFIG_ONU_FE_RESPIN_RF400_ETH
  atheros_4bytes_head.usHeaderType = 0x9100;
  atheros_4bytes_head.usVersionHi = 2;
  atheros_4bytes_head.usPriority = 3;
  atheros_4bytes_head.usPackType = 0;
  atheros_4bytes_head.usFromCpu = 1;
  atheros_4bytes_head.usPortNum = 0x1e;
#endif
...
static int __init onu_fe_probe(struct platform_device *pdev)
{
  ...
  for (i=0; i<MAX_INTETFACE_NUM; i++)
  {
    ...
    {
      wanndev = ndev;   /**wanndev and lanndev are the same in f13 **/
    }
