...
#ifdef CONFIG_ONU_FE_VOIP_F33_ETH
...
int onu_start_xmit_buf(struct net_device *ndev, u8 *sbuf, u32 len, u8 flg)
{
...
static int onu_start_xmit(struct sk_buff *skb, struct net_device *ndev)
{
  ...
  onu_start_xmit_buf(ndev, sbuf, len, 0);
}
...
static int dma_rx_receive(struct net_device *ndev, int budget)
{
  ...
  while ((received < budget) && find_valid_rx(received, fep, index, &valid_index))
  {
    ...
    else
    {
      ...
      for (i=devendnum-1, j=0; (i>=devstartnum) || (j==1); i--)
      {
        if (0 == j)
        {
          devtmp = onundevs[i];
        ...
        if ((with_vlan) && (i!=devnownum) && (0xF!=devnownum) && (0==j))
        {
          if (devstartnum == i)
          {
            devtmp = onundevs[MAX_INTETFACE_NUM-1];
          }
          else
          {
            devtmp = onundevs[i-1];
        ...
        if (devtmp)
        {
          ...
          if (MACEQUAL((char *)devtmp->dev_addr, (char *)dma_buf+offset0))
          {
            ...
            {
              if ((((u8 *)dma_buf)[35+offset] == 0x43) || (((u8 *)dma_buf)[36+offset] == 0x44))
              {
            ...
            {
            	printk("in onu_voip_fe_f33 0806\r\n");
              struct in_device *ipout=devtmp->ip_ptr;
              u32 iptmp, ipin;
...
{
#ifndef CONFIG_ONU_FE_VOIP_F33_ETH
