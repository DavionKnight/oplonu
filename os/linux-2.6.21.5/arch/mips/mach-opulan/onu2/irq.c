...
static unsigned long ipmux_irq_mask = 0x20c7e001;
...
void malta_hw0_irqdispatch(void)
{
	...
	u32 irq_enable0, irq_enable1;
	u32 irq_status0, irq_status1;
	
	irq_enable0 = onu_readl(MOD_INT_ENABLE_0);
	irq_enable1 = onu_readl(MOD_INT_ENABLE_1);

	irq_status0 = onu_readl(MOD_INT_STATUS_0) & irq_enable0;
	irq_status1= onu_readl(MOD_INT_STATUS_1) & irq_enable1;

	//prom_printf("%s, status=0x%x, status1=0x%x\n ",__FUNCTION__, irq_status, irq_status1);

	/*irq status 0*/
	if(irq_status0 & IRQ_BIT_DMA0) do_IRQ(OPL_HOSTDMA0_IRQ);
	if(irq_status0 & IRQ_BIT_DMA1) do_IRQ(OPL_FEDMA1_IRQ);
	if(irq_status0 & IRQ_BIT_PCM)  do_IRQ(OPL_ONU_PCM_IRQ);
	if(irq_status0 & IRQ_BIT_GPIO4) do_IRQ(OPL_ONU_GPIO_4);
	if(irq_status0 & IRQ_BIT_GPIO5) do_IRQ(OPL_ONU_GPIO_5);
	if(irq_status0 & IRQ_BIT_GPIO10) do_IRQ(OPL_ONU_GPIO_10);
	if(irq_status0 & ipmux_irq_mask){
		do_IRQ(OPL_IPMUX_IRQ);      /* for OTHERS,mainly for IPMUX-e switch chip */
		//printk("4)en:0x%x pd:0x%x.\n", irq_mask, irq_status);
		g_opl_chip_irq_pending = irq_status0 &ipmux_irq_mask;
	}

	/*irq status 1*/
	if(irq_status1 & IRQ1_BIT_UART0) do_IRQ(OPL_UART0_IRQ);
	if(irq_status1 & IRQ1_BIT_UART1) do_IRQ(OPL_UART1_IRQ);
	if(irq_status1 & IRQ1_BIT_TIMER0) do_IRQ(OPL_TIMER0_IRQ);
...
void ipmux_irq_enable(unsigned int irq)
{
	...
	volatile unsigned int irq_enable1 = 0;
	...
	switch(irq){
		...
		/*register 1*/
		case OPL_UART0_IRQ:   irq_enable1  |= (IRQ1_BIT_UART0);break; 
		case OPL_UART1_IRQ:   irq_enable1  |= (IRQ1_BIT_UART1);break;
		case OPL_TIMER0_IRQ:  irq_enable1  |= (IRQ1_BIT_TIMER0);break;
		default:
		printk("%s,%d:the irq %d is invalid\n",__FILE__,__LINE__,irq);
				   break;
	}

	//prom_printf("irq=%d, irq_enable=0x%x, irq_enable1=0x%x\n", irq, irq_enable, irq_enable1);
	/* enable bit*/
	*(volatile unsigned int *)(ONU_REG_BASE +MOD_INT_ENABLE_0) |= irq_enable;
	*(volatile unsigned int *)(ONU_REG_BASE +MOD_INT_ENABLE_1) |= irq_enable1;
...
void ipmux_irq_disable(unsigned int irq)
{
	...
	unsigned int irq_enable1 = 0;
	unsigned long flags;
	switch(irq){
		/*irq enable register 0*/
		case OPL_HOSTDMA0_IRQ:irq_enable |= (IRQ_BIT_DMA0);break;
		case OPL_FEDMA1_IRQ:irq_enable |= (IRQ_BIT_DMA1);break;
		case OPL_ONU_PCM_IRQ: irq_enable |= IRQ_BIT_PCM; break;
		case OPL_ONU_GPIO_4: irq_enable |= IRQ_BIT_GPIO4; break;
...
asmlinkage void plat_irq_dispatch()
{
	...
	//prom_printf("--------------%s, %s, pending=0x%x\n", __FILE__, __FUNCTION__, pending);
