...
OPL_STATUS dalStormClsRuleAdd(UINT32 stormLimitType, UP_DOWN_STREAM_e streamType)
{
	...
	if (*pRule > 0){
		OP_DEBUG(DEBUG_LEVEL_DEBUGGING, "[ %s ]: storm type %d stream type %d classify rule %d has been created.\r\n", __FUNCTION__, stormLimitType, streamType, *pRule);
		return OPL_OK;
	}
    ...
    OP_DEBUG(DEBUG_LEVEL_DEBUGGING, "[ %s ]: ruleId:%d, otherRuleId:%d, return code:%d.\r\n", __FUNCTION__, *pRule, *pOtherRule, ret);
...
OPL_STATUS dalStormClsRuleDel(UINT32 stormLimitType, UP_DOWN_STREAM_e streamType)
{
    ...
    OP_DEBUG(DEBUG_LEVEL_DEBUGGING, "[ %s ]: storm type %d stream type %d return code:%d.\r\n", __FUNCTION__, stormLimitType, streamType, ret);
...
OPL_STATUS dalStormCtrlFrameSet(UINT32 portId,UINT32 stormLimitType,UINT32 limitEnable)
{
    ...
    OP_DEBUG(DEBUG_LEVEL_DEBUGGING, "[ %s ]: portId:%d,stormLimitType:%d,limitEnable:%d.\r\n", __FUNCTION__, portId,stormLimitType,limitEnable);
	...
	OP_DEBUG(DEBUG_LEVEL_DEBUGGING, "[ %s ]: return %d.\r\n", __FUNCTION__, ret);
...
OPL_STATUS dalStormCtrlRateSet(UINT32 portId,UINT32 rateLimit)
{
    ...
    OP_DEBUG(DEBUG_LEVEL_DEBUGGING, "[ %s ]: portId:%d,rateLimit:%d.\r\n", __FUNCTION__, portId,rateLimit);
    ...
    if (1/*g_stromClsRuleAdded & BROADCAST_STORM_TYPE*/){
        ...
        OP_DEBUG(DEBUG_LEVEL_DEBUGGING, "[ %s ]: cir:%d, ret:%d.", __FUNCTION__, cir, ret);
        if ( OPL_OK != ret){
            OPL_LOG_TRACE();
            return OPL_ERROR;
        }
        ret = tmQueueCbsHwWrite(streamType, BRAODCAST_EN_QUEUE_NUM, cbs);
        OP_DEBUG(DEBUG_LEVEL_DEBUGGING, "[ %s ]: cbs:%d, ret:%d.", __FUNCTION__, cbs, ret);
        if ( OPL_OK != ret){
            OPL_LOG_TRACE();
            return OPL_ERROR;
        }
		if (IS_ONU_RESPIN)
		{
			...
			OP_DEBUG(DEBUG_LEVEL_DEBUGGING, "[ %s ]: pir:%d,pbs:%d, ret:%d.", __FUNCTION__, pir,pbs, ret);
			if ( OPL_OK != ret){
				OPL_LOG_TRACE();
				return OPL_ERROR;
			}
