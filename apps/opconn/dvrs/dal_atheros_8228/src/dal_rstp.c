...
OPL_STATUS dalRstpPortStateGet(IN UINT32 portId, OUT UINT8 * portState)
{
	...
	ulRet = shiva_stp_port_state_get(0, FAL_SINGLE_STP_ID, atherosPort, &ulState);
    if (SW_OK != ulRet)
    {
...
OPL_STATUS dalRstpPortStateSet(IN UINT32 portId, IN UINT8 portState)
{
    ...
    ulRet = shiva_stp_port_state_set(0, FAL_SINGLE_STP_ID, atherosPort, ulState);
...
OPL_STATUS dalRstpPortMacFlush(UINT32 portId, BOOL_T bOnlyThisPort)
{
	...
	{
		ulRet = shiva_fdb_del_by_port(0, atherosPort, 0);
        if (SW_OK != ulRet)
        {
    ...
    {
        for (ulCurPortid = 1; ulCurPortid < 5; ulCurPortid++)
        {
            ...
            ulRet = shiva_fdb_del_by_port(0, ulCurPortid, 0);
            if (SW_OK != ulRet)
            {
                printf("flush port %u fdb error %u\r\n", portId, ulRet);
                return ulRet;
