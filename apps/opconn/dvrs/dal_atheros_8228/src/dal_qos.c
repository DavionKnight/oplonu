...
OPL_STATUS dalQosGeInit(void)
{
	...
	shiva_qos_port_mode_set(0, 6, FAL_QOS_UP_MODE, 1);
	shiva_qos_port_mode_set(0, 6, FAL_QOS_DSCP_MODE, 1);

	/* to support downstream SP mode */
	shiva_port_pri_propagation_set(0, 6, 1);
	
	return shiva_qos_port_sch_mode_set(0, 6, FAL_SCH_SP_MODE, weight);
}
...
OPL_STATUS dalQosSchModeSet(a_uint32_t portId, fal_sch_mode_t mode)
{
	...
	shiva_qos_port_sch_mode_get(0, atherosPort, &tmpMode, weight);
	
	return shiva_qos_port_sch_mode_set(0, atherosPort, mode, weight);
}
...
OPL_STATUS dalQosSchModeGet(a_uint32_t portId, fal_sch_mode_t * mode)
{
	...
	return shiva_qos_port_sch_mode_get(0, atherosPort, mode, weight);
}
...
OPL_STATUS dalQosSchWtSet(a_uint32_t portId, a_uint32_t weight[])
{
	...
	shiva_qos_port_sch_mode_get(0, atherosPort, &mode, tmpWeight);
...
{
	return shiva_cosmap_up_queue_set(0, up, queue);
}
...
OPL_STATUS dalQosTagPri2QGet(a_uint32_t up, fal_queue_t * queue)
{
	...
	return shiva_cosmap_up_queue_get(0, up, queue);
}
...
{
	return shiva_cosmap_dscp_queue_set(0, dscp, queue);
}
