...
OPL_STATUS dalMulticastEnable()
{
	...
	{
		shiva_port_unk_mc_filter_set(0, port_id, 1);
	}
...
OPL_STATUS dalMulticastDisable()
{
	...
	/*allow mulcast flooding when igmp disabled */
	for(port_id = 1; port_id < 7; port_id++)
	{
		shiva_port_unk_mc_filter_set(0, port_id,0);
	}
	shiva_port_unk_mc_filter_set(0, 0, 1); /*forbit mulcast flooding for port 0 */

	shiva_igmp_mld_cmd_set(0, FAL_MAC_CPY_TO_CPU);
	...
	for(port_id = 1; port_id < 5; port_id++)
	{
		...
		{
			return shiva_port_egvlanmode_set(0, dalToAtherosPortMap[port_id], FAL_EG_HYBRID);
		}
		else if(VLAN_TAG == dal_vtt_info[dalToAtherosPortMap[port_id]].vlanMode)
		{
			return shiva_port_egvlanmode_set(0, dalToAtherosPortMap[port_id], FAL_EG_UNTAGGED);
		}
...
* 	SEE ALSO:
*/
OPL_STATUS dalMulticastVlanAddPort(UINT8 portId,UINT16 vlanId)
{
	fal_vlan_t vlan;
	fal_pbmp_t member;
...
OPL_STATUS dalMulticastVlanTagStripe(UINT8 portId,UINT8 enable)
{
	...
	if (enable)
	{
		return shiva_port_egvlanmode_set(0, atherosPort, FAL_EG_UNTAGGED);
	}
	else
	{
		return shiva_port_egvlanmode_set(0, atherosPort, FAL_EG_UNMODIFIED);
	}
}
