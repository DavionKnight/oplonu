...
OPL_STATUS dalSetLocalSwitch(UINT8 portId, UINT8  enable)
{
	...
	if (enable)
	{
		for (i = 1; i < 5; i++)
		{
		       if (atherosPort != i)
		       {
		            isis_portvlan_member_add(0, atherosPort, i);
		       }		
		}

		/* added by cylan for bug3027 */
		/* add the force_port_vlan_en=0 for the port not isolation */
		isis_port_force_portvlan_set(0, atherosPort, 0);
		...
		{
			case 1:
				isis_vlan_member_update(0, 4094, 0x5F, 0);
				dal_vlan8228[4094].member = 0x5F;
				break;
			case 2:
				isis_vlan_member_update(0, 4093, 0x5F, 0);
				dal_vlan8228[4093].member = 0x5F;
				break;
			case 3:
				isis_vlan_member_update(0, 4092, 0x5F, 0);
				dal_vlan8228[4092].member = 0x5F;
				break;
			case 4:
				isis_vlan_member_update(0, 4091, 0x5F, 0);
	...
	else
	{
		for (i = 1; i < 5; i++)
		{
				isis_portvlan_member_del(0, atherosPort, i);
			}

		/* added by cylan for bug3027 */
		/* add the force_port_vlan_en=1 for the port isolation */
		isis_port_force_portvlan_set(0, atherosPort, 1);

		/* add portvlan member = UNI + Mac6 */
		isis_portvlan_member_add(0, atherosPort, 6);
		//isis_portvlan_member_add(0, 6, atherosPort);
		...
		{
			case 1:
				isis_vlan_member_update(0, 4094, 0x43, 0);
				dal_vlan8228[4094].member = 0x43;				
				break;
			case 2:
				isis_vlan_member_update(0, 4093, 0x45, 0);
				dal_vlan8228[4093].member = 0x45;
				break;
			case 3:
				isis_vlan_member_update(0, 4092, 0x49, 0);
				dal_vlan8228[4092].member = 0x49;
	
				break;
			case 4:
				isis_vlan_member_update(0, 4091, 0x51, 0);
...
{
    UINT32  atherosPort = dalToAtherosPortMap[portId];
	return isis_port_default_cvid_set(0, atherosPort, vid);
}
...
{
    UINT32  atherosPort = dalToAtherosPortMap[portId];
	return isis_port_default_cvid_get(0, atherosPort, vid);
}
...
OPL_STATUS dalSetPortBaseVlanMember(UINT8 portId, UINT8 *memlist, UINT8 memnum)
{
	...
	for (num = 1; num < memnum; num++)
	{
	    if (memlist[num])
	    {
			isis_portvlan_member_add(0, atherosPort, 1 << num);
	    }
	}
...
OPL_STATUS dalGetPortBaseVlanMember(UINT8 portId, UINT8 *memlist, UINT8 *memnum)
{
	...
	isis_portvlan_member_get(0, atherosPort, &mem_port_map);
...
{
    UINT32  atherosPort = dalToAtherosPortMap[portId];
	return isis_port_1qmode_set(0, atherosPort, mode);
}
...
{
    UINT32  atherosPort = dalToAtherosPortMap[portId];
	return isis_port_1qmode_get(0, atherosPort, mode);
}
...
OPL_STATUS dalCreateVlanID(UINT16 vid)
{
	...
	if (SW_OK == rv)
	{
		...
		//isis_vlan_member_add(0, vid, 6, 0);
		return OPL_OK;
	}
	else
	{
...
OPL_STATUS dalDeleteVlanID(UINT16 vid)
{
	...
	if (SW_OK == isis_vlan_delete(0, vid))
	{
		memset(&dal_vlan8228[vid], 0 , sizeof(VTT_VLAN_8228_t));
...
OPL_STATUS dalAddVlanPort(UINT8 portId, UINT16 vid, VLAN_EGRESS_TYPE_E type)
{
	...
	switch(type){
		...
		case TAG:
			...
			break;
		default:
			egmode = FAL_EG_HYBRID;
			break;
	}
	
	if (SW_OK == isis_vlan_find(0, vid, &vlan))
	{
		member = vlan.mem_ports;
		member |= ((1 << atherosPort) + 0x1);	/* include GE port */
		
		rv =  isis_vlan_member_add(0, vid, atherosPort, egmode);
		rv =  isis_vlan_member_add(0, vid, 0, egmode);
		if (SW_OK == rv)
		{
...
OPL_STATUS dalMoveVlanPort(UINT8 portId, UINT16 vid)
{
	...
	if (SW_OK == isis_vlan_find(0, vid, &vlan))
	{
		...
		rv = isis_vlan_member_del(0, vid, atherosPort);
		if (SW_OK == rv)
		{
			dal_vlan8228[vid].member = member;
...
{
    UINT32  atherosPort = dalToAtherosPortMap[portId];
	return isis_port_egvlanmode_set(0, atherosPort, mode);
}
...
{
    UINT32  atherosPort = dalToAtherosPortMap[portId];
	return isis_port_egvlanmode_get(0, atherosPort, mode);
}
...
OPL_STATUS dalTrunkEntryAdd(UINT8 portId, UINT32 VlanId)
{
	...
	isis_port_default_cvid_get(0, atherosPort, &pcvid);
	if (VlanId == pcvid)
	{
	...
	rv = isis_vlan_find(0, VlanId, &p_vlan);
	if (SW_OK == rv)
	{
	...
	else
	{
		...
		isis_vlan_member_add(0, VlanId, 0, FAL_EG_TAGGED); /*6 to 0*/
	}

	rv = isis_vlan_member_add(0, VlanId, atherosPort, FAL_EG_TAGGED);

	
	//isis_vlan_member_add(0, VlanId, member, 0);
...
OPL_STATUS dalTrunkEntryDel(UINT8 portId, UINT32 VlanId)
{
	...
	if (SW_OK == isis_vlan_find(0, VlanId, &vlan))
	{
		member = vlan.mem_ports;
		member &= ~(1 << atherosPort);
		isis_vlan_member_del(0, VlanId, atherosPort);

	
		//rv =  isis_vlan_member_add(0, VlanId, member, 0);
		//if (SW_OK == rv)
		{
			dal_vlan8228[VlanId].member = member;
...
dalVlanShow(UINT32	vid)
{
	...
	{
		printf("On chip: vid = %d, type = %d, fid = %d, port member = %X, tagged_ports = %X, untagged_ports = %X, unmodify_ports = %X, uport = %X\n", 
			vid, dal_vlan8228[vid].type, p_vlan.fid, p_vlan.mem_ports, p_vlan.tagged_ports, p_vlan.untagged_ports, p_vlan.unmodify_ports, p_vlan.u_ports);
	}
