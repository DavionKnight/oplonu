...
//isis_acl_list_t *sw_list_ent[];//hujian here
...
OPL_STATUS dalAcl1to1VtAdd(UINT32 portId, UINT32 cVlan, UINT32 sVlan, UINT32 * index)
{
	...
	rv = isis_acl_list_creat(0, listId, 0);
	if (SW_OK != rv)
	{
		return rv;
	}
	...
	rv = isis_acl_rule_add(0, listId, 0, 1, &rule);
	if (SW_OK != rv)
	{
		isis_acl_list_destroy(0, listId);
		return rv;
	}


	/* bind ACL list to portId */
	rv = isis_acl_list_bind(0, listId, 0, 0, atherosPort);
	if (SW_OK != rv)
	{
		isis_acl_rule_delete(0, listId, 0, 1);
		isis_acl_list_destroy(0, listId);
		return rv;
	}		
	...
	if (SW_OK != rv)
	{
		...
		isis_acl_rule_delete(0, usListid, 0, 1);
		isis_acl_list_unbind(0, usListid, 0, 0, atherosPort);
		isis_acl_list_destroy(0, usListid);
	...
	rv = isis_acl_list_bind(0, listId, 0, 0, DAL_ACL_UPLINK_PORT);
	if (SW_OK != rv)
	{
		isis_acl_rule_delete(0, listId, 0, 1);
		isis_acl_list_destroy(0, listId);
...
OPL_STATUS dalAcl1to1VtDel(UINT32 portId, UINT32 cVlan, UINT32 sVlan, UINT32 * index)
{
	...
	isis_acl_list_unbind(0, listId, 0, 0, atherosPort);
	isis_acl_rule_delete(0, listId, 0, 1);
	isis_acl_list_destroy(0, listId);
	dalAclPool[loc].aclIdUs[i] = DAL_ACL_INVALID_LIST_ID;
	dalAclPool[loc].fromVid[i] = 0;
	...
	isis_acl_list_unbind(0, listId, 0, 0, DAL_ACL_UPLINK_PORT);
	isis_acl_rule_delete(0, listId, 0, 1);
	isis_acl_list_destroy(0, listId);
...
OPL_STATUS dalAclNto1MacVtDel(UINT32 portId, UINT32 cVlan, UINT32 sVlan, UINT8 * mac, UINT32 * index)
{
	...
	listId = dalAclPool[loc].aclIdUs[i];
	isis_acl_list_unbind(0, listId, 0, 0, atherosPort);
//	isis_acl_rule_delete(0, listId, sw_list_ent[0][listId].size, 1);
	isis_acl_list_destroy(0, listId);
...
OPL_STATUS dalAclClsAdd(CLS_CONFIG_INFO_t *stClassCfg, UINT16 *ruleId)
{
	...
	if (OPL_OK == dalAclFindCls(stClassCfg->name, &tmpRuleId))
	{
		...
		isis_acl_rule_delete(0, listId, 0, 1);
		isis_acl_list_unbind(0, listId, 0, 0, dalAclPool[tmpRuleId].port);
		isis_acl_list_destroy(0, listId);		
	...
	for(i = 1; i < DAL_ACL_UPLINK_PORT; i++)
	{
		...
		{			
			tmpPortMap |= (1 << dalToAtherosPortMap[i]);
			rv = isis_acl_list_bind(0, listId, 0, 0, dalToAtherosPortMap[i]);
		}
	}
...
OPL_STATUS dalAclClsDel2(UINT16 ruleId)
{
	...
	while(j < DAL_ACL_UPLINK_PORT)
	{
		if ((tmpPortMap >> j) & 1)
		{
			isis_acl_list_unbind(0, listId, 0, 0,j);
		}
		j++;
	}
	isis_acl_rule_delete(0, listId, 0, 1);
...
OPL_STATUS dalAclIgmpAdd(void)
{
	...
	for(i = 1; i < DAL_ACL_UPLINK_PORT; i++)
	{
		...
		/* rv = isis_acl_list_bind(0, listId, 0, 0, i); */
	}
...
OPL_STATUS dalAclRstpAdd(UINT8 flg)  //0:cpu 1:ge
{
    ...
    listId = RstplistId;
	  ...
	  for(i = 1; i < DAL_ACL_UPLINK_PORT; i++)
	  {
		  isis_acl_list_unbind(0, listId, 0, 0, i);
		}
		isis_acl_list_destroy(0, listId);
	  RstplistId = listId;
	}
...
OPL_STATUS dalAclIgmpEnable(a_bool_t enable)
{
	...
	{     
		...
		if (enable)
		{
			isis_acl_list_bind(0, igmpListId, 0, 0, i); 
		}
		else
		{
			isis_acl_list_unbind(0, igmpListId, 0, 0, i); 
		}
	}
...
OPL_STATUS dalAclCtcVTAdd(UINT16 oldVid, UINT16 newVid, UINT8 portID)
{
	...
	rv = isis_acl_list_bind(0, listId, 0, 0, 6);
	if (SW_OK != rv)
	{
		isis_acl_rule_delete(0, listId, 0, 1);
		isis_acl_list_destroy(0, listId);
...
OPL_STATUS dalAclCtcVTDel(UINT16 oldVid, UINT16 newVid, UINT8 portID)
{
	...
	rv = isis_acl_list_unbind(0, listId, 0, 0,6);
	if (SW_OK != rv)
	{
...
OPL_STATUS dalAclZeroMacDiscard(void)
{
	...
	for (j=0; j<2; j++)
	{
		...
		if (SW_OK != rv)
		{
			printf("isis_acl_list_creat failed %d.\n", rv);
			return rv;
		}
		...
		{
			isis_acl_list_destroy(0, listId);
			//printf("isis_acl_rule_add failed %d.\n", rv);
			return rv;
		}
		...
		if (SW_OK != rv)
		{
			isis_acl_rule_delete(0, listId, 0, 1);
			isis_acl_list_destroy(0, listId);
			//printf("isis_acl_list_bind failed %d.\n", rv);
			return rv;
		}		
...
OPL_STATUS dalAclOnuMacAdd(UINT8 * oldMac,UINT8 * mac)
{
	UINT8	listId, i, j;
	fal_acl_rule_t  rule;
	UINT32  hsl_f_rsc = 0;
	...
	//printf("dalAclOnuMacAdd called oldmac %x.\n", oldMac);

	if (oldMac){
		if (DAL_ACL_MAC_SAME(oldMac, mac)){
			return OPL_OK;
		}
		for (i = 0; i < DAL_ACL_TOTAL_NUM; i++){
			if (DAL_ACL_FDB == dalAclPool[i].type && 
				DAL_ACL_MAC_SAME(oldMac, dalAclPool[i].mac)){
				//printf("acl delete old mac.\n");
					/* old rule exists,  delete it */
				listId = dalAclPool[i].aclIdUs[0];
				tmpPortMap = dalAclPool[i].port;
				...
				vosMemSet(&dalAclPool[i], 0, sizeof (DAL_ACL_INFO_s));
				dalAclPool[i].type = DAL_ACL_UNUSED;	
				dalAclPool[i].aclIdUs[0] = DAL_ACL_INVALID_LIST_ID;
				break;
			}
		}
	}
	...
	for (j=0; j<1; j++)
	{
		...
		{	
			...
			rule.rule_type = FAL_ACL_RULE_MAC;
			vosMemSet(rule.dest_mac_mask.uc, 0xFF, sizeof(fal_mac_addr_t));
			vosMemCpy(rule.dest_mac_val.uc, mac, 6);
			FAL_FIELD_FLG_SET(rule.field_flg,FAL_ACL_FIELD_MAC_DA);
		}
		...
		dalAclPool[tmpRuleId].type = DAL_ACL_FDB; 
		dalAclPool[tmpRuleId].aclIdUs[0] = listId;
		dalAclPool[tmpRuleId].port = 0x7E;
		vosMemCpy(dalAclPool[tmpRuleId].mac, mac, 6);
	}
	
	return OPL_OK;	
}

OPL_STATUS dalPortFlowctrlAclSet(a_uint32_t dev_id, fal_port_t port_id,
                                                 a_bool_t enable)
{
    ...
    a_uint32_t policer_id;
	
    list_id = DAL_ACL_PORT_FLOWCTRL_PORT1_LIST_ID + port_id - 1;
    policer_id = DAL_ACL_INGRESS_RATELIMIT_PORT1_POLICY_ID + port_id - 1;
  if(1 == enable)
  {
    memset(&policer,0,sizeof(fal_acl_policer_t));   
    isis_rate_acl_policer_get(0, policer_id, &policer);
    policer.cbs = 8 * 1024 * 1024;
    policer.meter_interval = FAL_RATE_MI_100US;
    status=isis_rate_acl_policer_set(0,policer_id,&policer);
    ...
    /* bind acl list to port */
    status = isis_acl_list_bind(0,list_id,FAL_ACL_DIREC_IN,FAL_ACL_BIND_PORT,port_id);
    if(status != OPL_OK)
    {
  ...
  { 
    memset(&policer,0,sizeof(fal_acl_policer_t));   
    isis_rate_acl_policer_get(0, policer_id, &policer);
    policer.cbs = 0xffffffff;
    policer.meter_interval = FAL_RATE_MI_1MS;
    status=isis_rate_acl_policer_set(0,policer_id,&policer);
	
    status = isis_acl_list_destroy(0,list_id);
    if(status != SW_OK)
    {
    	//printf("\n isis_acl_list_destroy error, return %d! file %s, line %d\n", status,__FILE__,__LINE__);    	
    }
    return status;
  }
}
...
a_uint32_t CIR_value = 0;
fal_acl_policer_t policer;	
UINT8  flowctrlEnable;

	if(acl_policer_id >= DAL_ACL_POLICER_NUM)
	{
		printf("\n acl_policer_id %d error, file %s, line %d\n",acl_policer_id,__FILE__,__LINE__);
		return SW_OUT_OF_RANGE;
	}

	if((kbits_sec>0)&&(kbits_sec < 32))
  	{
		kbits_sec = 32;
  	}
	if(kbits_sec == 0)
	...
	status=isis_rate_acl_policer_set(0,acl_policer_id,&policer);

	if(status!= SW_OK)
	{
		printf("\n isis_rate_port_policer_get return Failed,error num %d, file %s, line %d\n",status,__FILE__,__LINE__);
		return status;				
	}	
	return SW_OK;
}
...
OPL_STATUS dalAclInit()
{
	...
	//dalAclIgmpAdd();    //for s17, IGMP packet will be trapped on the opconn chip
	dalAclRstpAdd(0);

	dalAclZeroMacDiscard();

       /* init acl policer for s17 flow control */
	for(i=0; i<4; i++)
	{
           dalAclIngressRatelimitSet(i, DAL_ACL_INGRESS_RATELIMIT_PORT1_POLICY_ID+i, 0);   
	}
	   
	vosHWAddrGet("eth0", &hwAddr[0]);
	dalAclOnuMacAdd(NULL, hwAddr);
