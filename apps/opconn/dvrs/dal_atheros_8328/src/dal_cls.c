...
OPL_STATUS dalClsTblEntryRemove(opl_cls_hwtbl_t *clstblshadow)
{
	...
	vosMemSet(&(gpCmaskTbl[hwmaskID]),0, sizeof(CLASSIFY_MASK_ENTRY_t));
	iStatus = dalClsMaskHwWr(&(gpCmaskTbl[hwmaskID]), hwmaskID);
		
	vosMemSet(&(gpCruleTbl[hwruleID]), 0,sizeof(CLASSIFY_RULE_ENTRY_t));
	dalClsRuleHwWr(&(gpCruleTbl[hwruleID]), hwruleID);
	
	vosMemSet(&(gpCaddrTbl[address].ClasAddr[addrOff]), 0,sizeof(UINT16));
...
OPL_STATUS dalClsTblEntryChange(UINT16 hwoldmaskID,UINT16 hwoldruleID,UINT16 hwnewmaskID)
{
  ...
  vosMemSet(&(gpCmaskTbl[hwoldmaskID]),0, sizeof(CLASSIFY_MASK_ENTRY_t));
...
OPL_STATUS dalClsRuleEntryShow(int fd,UINT16 startId,UINT16 endId,UINT8 mode)
{
  ...
  if (startId >= TAB_CLS_RULE_LEN || endId >= TAB_CLS_RULE_LEN) {
    OPL_DAL_PRINTF(("entryId out of rule table range.\r\n"));
    return OPL_ERR_INVALID_PARAMETERS;
  }

  DAL_CLS_SHOW(fd,
               "%-3s %-4s %-6s %-7s %-6s %-6s %-7s %-4s %-6s %s\r\n",
               "No",
               "Tact",
               "egress",
               "ingress",
  ...
  if ((mode == CLASSIFY_DAL_TABLE) || (mode == CLASSIFY_DAL_AND_ASIC_TABLE)) {
    DAL_CLS_SHOW(fd, "------------------------------------\r\n");
    DAL_CLS_SHOW(fd, "Asic rule entry count: %d/128\n", l_count);
  }
  if ((mode == CLASSIFY_ASIC_TABLE) || (mode == CLASSIFY_DAL_AND_ASIC_TABLE)) {
    DAL_CLS_SHOW(fd, "------------------------------------\r\n");
    DAL_CLS_SHOW(fd, "Asic rule entry count: %d/128\r\n", c_count);
...
OPL_STATUS dalClsMaskEntryShow(int fd,UINT16 startId,UINT16 endId,UINT8 mode)
{
  ...
  if (startId >= TAB_CLS_MASK_LEN || endId >= TAB_CLS_MASK_LEN) {
    OPL_DAL_PRINTF(("entryId out of mask table range.\r\n"));
    return OPL_ERR_INVALID_PARAMETERS;
  }

  DAL_CLS_SHOW(fd,
               "%-3s %-5s %-5s %-5s %-3s %-3s %-3s %-3s %-3s %-3s %-3s %-3s %s\r\n",
               "No",
               "valid",
               "imask",
               "emask",
  ...
  for (entryId = startId; entryId <= endId; entryId++) {
    if ((mode == CLASSIFY_DAL_TABLE) || (mode == CLASSIFY_DAL_AND_ASIC_TABLE)) {
      ...
      if (pDalClsMaskEntry->bfValid == 1) {
        //OPL_DAL_PRINTF(("Local:\n"));
        DAL_CLS_SHOW(fd,
                     "%-3d %-5x %-5x %-5x %-3d %-3d %-3d %-3d %-3d %-3d %-3d %-3d\r\n",
                     entryId,
  ...
  if ((mode == CLASSIFY_DAL_TABLE) || (mode == CLASSIFY_DAL_AND_ASIC_TABLE)) {
    DAL_CLS_SHOW(fd, "------------------------------------\r\n");
    DAL_CLS_SHOW(fd, "Asic mask entry count: %d/23\n", l_count);
  }
  if ((mode == CLASSIFY_ASIC_TABLE) || (mode == CLASSIFY_DAL_AND_ASIC_TABLE)) {
    DAL_CLS_SHOW(fd, "------------------------------------\r\n");
    DAL_CLS_SHOW(fd, "Asic mask entry count: %d/23\n", c_count);
...
OPL_STATUS dalClsAddrEntryShow(int fd,UINT16 startId,UINT16 endId,UINT8 mode)
{
  ...
  if (startId >= TAB_CLS_ADDR_LEN || endId >= TAB_CLS_ADDR_LEN) {
    OPL_DAL_PRINTF(("entryId out of addr table range.\r\n"));
    return OPL_ERR_INVALID_PARAMETERS;
  }
                             ...
                             UINT8 pktType)
{
 ...
 int i;
  ...
  if (stClassCfg->queueMapId < CTC_QUEUE_MAP_7) {
		...
		{
//			printf("-ltang debug-! stClassCfg->rmkicoschange is %d",stClassCfg->rmkicoschange);
			pParameter->RuleEntry.bfRmkAct |= CHANGE_ICOS_FIELD;
		}
		/*modified by ltang,for bug3222 ---end*/
  ...
  */
  ...
  if (stClassCfg->rmkvidOp == 0x2 || stClassCfg->rmkvidOp == 0x3) {
...
OPL_STATUS dalClsRuleCtcAdd(CLS_CONFIG_INFO_t *stClassCfg,UINT16 *ruleId)
{
	...
	return dalAclClsAdd(stClassCfg, ruleId);
}
...
OPL_STATUS dalOnuClsRuleCtcAdd(opl_acl_list_t *oamcls)
{
  ...
  for (ruleIndex = OPL_ZERO;
	...
	//printf("add index %d for precedence %d \n",firstEmptyRuleId 
		//,stClassCfg->rulePri);
  if (firstEmptyRuleId != 0xffffffff) {
    pClassItemInfo = &(gClassifyItem[firstEmptyRuleId]);
  }
...
/* Bug3058 F23 ,modified by cxji,2010/06/17 14:00:00 .begin*/
    for (ruleIndex = OPL_ZERO;
       ruleIndex < CLASSIFY_MAX_ITEM_NUM- MAX_GROUP_NUM  ;
       ruleIndex++) {
    	if(gClassifyItem[ruleIndex].valid && !OPL_MEMCMP((UINT8 *)
                        ...
                        // printf("rule data same %d  empty %d  ,return \n",ruleIndex,firstEmptyRuleId);
	    OPL_LOG_TRACE();
		return OPL_ERROR;
		}
  } 
/* Bug3058 F23 ,modified by cxji,2010/06/17 14:00:00 .end*/
...
OPL_STATUS dalOnuClsRuleCtcDel(opl_acl_list_t *oamcls)
{
  ...
  opl_cls_hwtbl_t clstblshadow;
	...
	vosMemCpy(&clstblshadow,&(gClassifyItem[ruleIndex].shadowtbl),sizeof(opl_cls_hwtbl_t));
...
OPL_STATUS dalClsRuleCtcDel(CLS_CONFIG_INFO_t *stClassCfg,UINT16 *ruleId)
{
	...
	return dalAclClsDel(stClassCfg, ruleId);
}
...
void dalClsCfgShow(int fd,UINT16 usFirstItem,UINT16 usLastItem)
{
  UINT16 usCbitMapCnt;
  UINT16 usCfgCnt;
  UINT16 usShowCnt = 0;
  ...
  if (0 == usFirstItem) {
    OPL_DAL_PRINTF(("Classification ID cant be set zero!\r\n"));
    return;
  }
  ...
  for (usCfgCnt = usFirstItem - 1; usCfgCnt < usLastItem; usCfgCnt++) {
    if (1 == gClassifyItem[usCfgCnt].valid) {
      ucFlag = 1;    
      usShowCnt = 0;
      DAL_CLS_SHOW(fd, "ClassifyNum : %-5d", usCfgCnt + 1);
      DAL_CLS_SHOW(fd, "Rule Precendence : %-2d", gClassifyItem[usCfgCnt].Parameter.maskPri);
      DAL_CLS_SHOW(fd,
                   "RuleEntry   : %-5d(%d)",
      ...
      DAL_CLS_SHOW(fd, "PacketType  : ");
      switch (gClassifyItem[usCfgCnt].Parameter.pktType) {
        case CLASSIFY_EII_IP:
          DAL_CLS_SHOW(fd, "Ethernet.\r\n");
             ...
             != (gClassifyItem[usCfgCnt].Parameter.FieldMap.bitMap & (CLASSIFY_FIELD_MS_BIT
              ...
              switch (usCbitMapCnt) {
                case CLASSIFY_SOURCEMAC:
                  DAL_CLS_SHOW(fd,
                               "SourceMac      : %02x:%02x:%02x:%02x:%02x:%-17x",
                ...
                case CLASSIFY_DESTINATIONMAC:
                  DAL_CLS_SHOW(fd,
                               "DestinationMac : %02x:%02x:%02x:%02x:%02x:%-17x",
                ...
                case CLASSIFY_SOURCE_IP:
                  ...
                  sprintf(aucAddr,
                          ...
                          pucIpAddr[2],
                          pucIpAddr[3]);
                  DAL_CLS_SHOW(fd, "IpSourceIpAddr : %-22s", aucAddr);
                  usShowCnt++;
                  break;
                case CLASSIFY_DESTINATION_IP:
                  ...
                  sprintf(aucAddr,
                          ...
                          pucIpAddr[2],
                          pucIpAddr[3]);
                  DAL_CLS_SHOW(fd, "IpDestIpAddr   : %-22s", aucAddr);
                  usShowCnt++;
                  break;
