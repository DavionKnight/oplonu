...
static inline sw_error_t
_shiva_port_autoneg_done_read(a_uint32_t dev_id, fal_port_t port_id, 
                                              a_bool_t * status)
{
    sw_error_t rv;
    a_uint32_t phy_id;
    ...
    *status = f2_autoneg_done(dev_id, phy_id);
...
#if 0  /* deleted by Gan Zhiheng, for half flow control - 2010/11/10 */
    SW_SET_REG_BY_FIELD(PORT_STATUS, TX_HALF_FLOW_EN, 0, reg);
#endif /* #if 0 */
 ...
 */
HSL_LOCAL sw_error_t
shiva_phy_autoneg_done_read (a_uint32_t dev_id, fal_port_t port_id,
                                  a_bool_t * status)
{
    sw_error_t rv;

    HSL_API_LOCK;
    rv = _shiva_port_autoneg_done_read(dev_id, port_id, status);
    HSL_API_UNLOCK;
    return rv;  
}
/* end: add by xukang for bug 3265 */
...
/**
 * @brief Disable auto negotiation status on a particular port.
 * @param[in] dev_id device id
 * @param[in] port_id port id
 * @return SW_OK or error code
 */
HSL_LOCAL sw_error_t
shiva_port_autoneg_disable(a_uint32_t dev_id, fal_port_t port_id)
{
    sw_error_t rv;
    a_uint32_t phy_id;
    ...
    rv = f2_phy_disable_autoneg(dev_id, phy_id);
    return rv;
}
                                   ...
                                   a_bool_t enable)
{
	...
	rv = shiva_port_autoneg_adv_get(dev_id,port_id, &autoadv);
	...
	rv = shiva_port_autoneg_adv_set(dev_id,port_id, autoadv);		
 ...
 */
HSL_LOCAL sw_error_t
shiva_port_lpbk_set(a_uint32_t dev_id, fal_port_t port_id,
                           a_bool_t enable)
{
    sw_error_t rv;
    a_uint32_t val;
    ...
    HSL_REG_FIELD_SET(rv, dev_id, PORT_CTL, port_id, MAC_LOOP_BACK,
                      (a_uint8_t *) (&val), sizeof (a_uint32_t));

    rv = hsl_port_prop_get_phyid(dev_id, port_id, &phy_id);

	f2_phy_set_lpbk(dev_id, phy_id, enable);
 ...
 */
HSL_LOCAL sw_error_t
shiva_port_lpbk_get(a_uint32_t dev_id, fal_port_t port_id,
                           a_bool_t * enable)
{
    sw_error_t rv;
    a_uint32_t val;
    ...
    HSL_REG_FIELD_GET(rv, dev_id, PORT_CTL, port_id, MAC_LOOP_BACK,
                      (a_uint8_t *) (&val), sizeof (a_uint32_t));
    SW_RTN_ON_ERROR(rv);
...
/**
 * @brief Set Phy shutdown on a particular port.
 * @param[in] dev_id device id
 * @param[in] port_id port id
 * @param[in] enable A_TRUE or A_FALSE
 * @return SW_OK or error code
 */
HSL_LOCAL sw_error_t
shiva_port_shutdown_set(a_uint32_t dev_id, fal_port_t port_id,
                           a_bool_t enable)
{
    sw_error_t rv;
    a_uint32_t val;
	...
	if (enable)
	{
        //printf("shiva_port_shutdown_set: f2_phy_poweroff(%d)\n", phy_id);
		f2_phy_poweroff(dev_id, phy_id);
	}
	else
	{
        //printf("shiva_port_shutdown_set: f2_phy_poweron(%d)\n", phy_id);
		f2_phy_poweron( dev_id, phy_id);
	}
...
HSL_LOCAL sw_error_t
shiva_port_shutdown_set_only(a_uint32_t dev_id, fal_port_t port_id,
                           a_bool_t enable)
{
    sw_error_t rv;
    a_uint32_t val;
	...
	if (enable)
	{
		f2_phy_poweroff_only(dev_id, phy_id);
	}
	else
	{
		f2_phy_poweron( dev_id, phy_id);
        ...
        a_bool_t *up)
{
    ...
    *up = f2_phy_get_link_status(dev_id, phy_id);
 ...
 */
HSL_LOCAL sw_error_t
shiva_port_admin_status_get(a_uint32_t dev_id, fal_port_t port_id,
        a_bool_t *up)
{
    sw_error_t rv;
    a_uint32_t phy_id;
    ...
    *up = f2_phy_get_power(dev_id, phy_id);
