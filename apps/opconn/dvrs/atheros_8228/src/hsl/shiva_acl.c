...
shiva_acl_list_t *list_ent[SW_MAX_NR_DEV];
static shiva_acl_hw_rule_t *hw_rule_ent;

a_uint32_t filter[SW_MAX_NR_DEV];
a_uint32_t filter_snap[SW_MAX_NR_DEV];
                         ...
                         shiva_acl_hw_rule_t * hw)
{
    ...
    if ((FAL_ACTION_FLG_TST(sw->action_flg, FAL_ACL_ACTION_MODIFY_VLAN))
        ...
        if (FAL_ACTION_FLG_TST(sw->action_flg, FAL_ACL_ACTION_MODIFY_VLAN)) {
			...
			/* add disable replace vlan lookup */
            if (FAL_ACTION_FLG_TST(sw->action_flg, FAL_ACL_ACTION_VID_MEM_DIS)) {
                SW_SET_REG_BY_FIELD(ACL_RSLT1, VID_MEM_EN, 0, hw->act[1]);
            }			
        }
    }
                      ...
                      a_uint32_t dst_slct_idx, a_uint32_t size)
{
    ...
    for (i = 0; i < size; i++) {
		...
		rv = _shiva_acl_filter_read(dev_id, &rule, (a_uint32_t ) src_idx);
        SW_RTN_ON_ERROR(rv);
 
        rv = _shiva_acl_filter_write(dev_id, &rule, (a_uint32_t ) dst_idx);
        SW_RTN_ON_ERROR(rv);
