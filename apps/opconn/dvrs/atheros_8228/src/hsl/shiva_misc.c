...
static inline sw_error_t
_shiva_mgmt_vid_vio_drop_set(a_uint32_t dev_id, a_bool_t enable)
{
	sw_error_t rv;
	a_uint32_t val;
	...
	HSL_REG_FIELD_SET(rv, dev_id, QM_CTL, 0, MANAGE_VID_VIO_DROP_EN,
					  (a_uint8_t *) (&val), sizeof (a_uint32_t));
	return rv;
}
...
static inline sw_error_t
_shiva_loop_check_intr_clear(a_uint32_t dev_id)
{
    sw_error_t rv;
    a_uint32_t data = 1;
    ...
    HSL_REG_FIELD_SET(rv, dev_id, GLOBAL_INT, 0, GLB_LOOP_CHECK,
                      (a_uint8_t *) (&data), sizeof (a_uint32_t));
    SW_RTN_ON_ERROR(rv);
...
static inline sw_error_t
_shiva_loop_check_intr_get(a_uint32_t dev_id, a_bool_t * intr)
{
    sw_error_t rv;
    a_uint32_t data = 0;
    ...
    HSL_REG_FIELD_GET(rv, dev_id, GLOBAL_INT, 0, GLB_LOOP_CHECK,
                      (a_uint8_t *) (&data), sizeof (a_uint32_t));
    SW_RTN_ON_ERROR(rv);
...
/**
 * @brief Set management packets vid violation drop.
 * @details     comments:
 ...
 */
HSL_LOCAL sw_error_t
shiva_mgmt_vid_vio_drop_set(a_uint32_t dev_id, a_bool_t enable)
{
    sw_error_t rv;

    HSL_API_LOCK;
    rv = _shiva_mgmt_vid_vio_drop_set(dev_id, enable);
    HSL_API_UNLOCK;
    return rv;
}
 ...
 */
HSL_LOCAL sw_error_t
shiva_loop_check_intr_get(a_uint32_t dev_id, a_bool_t * intr)
{
    sw_error_t rv;

    HSL_API_LOCK;
    rv = _shiva_loop_check_intr_get(dev_id, intr);
    HSL_API_UNLOCK;
    return rv;
}

/**
 * @brief Clear loopback checking status on a particular device.
 * @param[in] dev_id device id
 * @param[out] enable A_TRUE or A_FALSE
 * @return SW_OK or error code
 */
HSL_LOCAL sw_error_t
shiva_loop_check_intr_clear(a_uint32_t dev_id)
{
    sw_error_t rv;

    HSL_API_LOCK;
    rv = _shiva_loop_check_intr_clear(dev_id);
    HSL_API_UNLOCK;
    return rv;
}
