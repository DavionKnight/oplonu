                    ...
                    a_uint8_t value[], a_uint32_t value_len)
{
    ...
    rv = sd_reg_mdio_set(phy_addr, phy_reg, phy_val);
    SW_RTN_ON_ERROR(rv);
    ...
    rv = sd_reg_mdio_get(phy_addr, phy_reg, &tmp_val);
    SW_RTN_ON_ERROR(rv);
    reg_val = tmp_val;
                   ...
                   a_uint32_t reg, a_uint16_t * value)
{
    ...
    MDIO_LOCKER_LOCK;
    rv = sd_reg_mdio_get(phy_addr, reg, value);
    MDIO_LOCKER_UNLOCK;
                   ...
                   a_uint32_t reg, a_uint16_t value)
{
    ...
    MDIO_LOCKER_LOCK;
    rv = sd_reg_mdio_set(phy_addr, reg, value);
    MDIO_LOCKER_UNLOCK;
...
sw_error_t atheros_show_reg(a_uint32_t reg_addr)
{
	...
	shiva_reg_get(0, reg_addr, (a_uint8_t *) &value, sizeof(a_uint32_t));
...
{
	shiva_reg_set(0, reg_addr, (a_uint8_t *) &value, sizeof(a_uint32_t));
}

sw_error_t atheros_show_phy(a_uint32_t phy_addr, a_uint8_t reg)
{
	a_uint16_t value;
	
	shiva_phy_get(0, phy_addr, reg, &value);

	printf("atheros phy addr = 0x%X, reg = %d, value = 0x%X\n",
		phy_addr, reg, value);
}

sw_error_t atheros_wr_phy(a_uint32_t phy_addr, a_uint8_t reg, a_uint16_t value)
{	
	shiva_phy_set(0, phy_addr, reg, value);
}
