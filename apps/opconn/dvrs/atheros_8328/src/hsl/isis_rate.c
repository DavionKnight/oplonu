...
{
    sw_error_t rv;
    a_uint32_t cir = 0x7fff, eir = 0x7fff, cbs, ebs, tmp, data[3] = { 0 };

    data[0] = 0x18000000;
	//printf("port_id %d c_enable %d e_enable %d e_rate_flag %x eir %x.\n", port_id,policer->c_enable, policer->e_enable, policer->e_rate_flag, policer->eir);
	...
	//printf("----%s c_enable %d e_enable %d e_rate_flag 0x%x eir 0x%x.\n", __FUNCTION__,policer->c_enable, policer->e_enable, policer->e_rate_flag, eir);
...
{
    sw_error_t rv;
    a_uint32_t unit, data, cir, eir, cbs = 0, ebs = 0;
...
{
    sw_error_t rv;
    a_uint32_t ts, cir, eir, cbs, ebs, addr, data[2] = { 0 };
