...
//printf("%s vid %d portId %d portinfo %d.\n", __FUNCTION__, vlan_id, port_id, port_info);
/*	for (i=0; i<7; i++){
		if ((port_id>>i)&0x1){
			rv = _isis_vlan_member_update(dev_id, vlan_id, i, info);
		}
		else {
			_isis_vlan_member_update(dev_id, vlan_id, i, 0x3);
		}
...
_isis_vlan_member_del(a_uint32_t dev_id, a_uint32_t vlan_id, fal_port_t port_id)
{
    sw_error_t rv;
    a_uint32_t info = 0x3;
//printf("%s vid %d portId %d.\n", __FUNCTION__, vlan_id, port_id);
...
isis_vlan_create(a_uint32_t dev_id, a_uint32_t vlan_id)
{
	...
	//printf("%s vid %d created.", __FUNCTION__, vlan_id);
...
HSL_LOCAL sw_error_t
isis_vlan_create_svl(a_uint32_t dev_id, a_uint32_t vlan_id)
{
    sw_error_t rv;
	a_uint32_t reg[2] = { 0 };


	//printf("%s svl vid %d created.", __FUNCTION__, vlan_id);
    ...
    SW_SET_REG_BY_FIELD(VLAN_TABLE_FUNC0, VT_VALID, 1, reg[0]);
    SW_SET_REG_BY_FIELD(VLAN_TABLE_FUNC0, IVL_EN, 0, reg[0]); /*create svl vlan entry.*/
    SW_SET_REG_BY_FIELD(VLAN_TABLE_FUNC0, LEARN_DIS, 0, reg[0]);
    SW_SET_REG_BY_FIELD(VLAN_TABLE_FUNC0, VID_MEM, 0x3fff, reg[0]);
    SW_SET_REG_BY_FIELD(VLAN_TABLE_FUNC1, VLAN_ID, vlan_id, reg[1]);
