...
OPL_STATUS ponSecuNoCtcKeyGenerate(UINT8 keyIndex)
{
	...
	srand(123+randVecTor*0X123);
	keyValUnion.keyVal8[1] = (rand()+randVecTor)&0xff;
	
	srand(238+randVecTor*458);
	keyValUnion.keyVal8[2] = keyArry[(rand()+randVecTor)%5];

	srand(0xffff+randVecTor*666);
	keyValUnion.keyVal8[3] = keyArry[(rand()+randVecTor)%5];
...
void ponSerdesReset()
{
    ...
    *(UINT32 *)((UINT32)regBaseAddr + REG_PMAC_SERDES_COMMON) = 0x0000056c;
    vosUDelay(10000);

    /* oplRegWrite(REG_PMAC_SERDES_COMMON, 0x8000056c); */
    *(UINT32 *)((UINT32)regBaseAddr + REG_PMAC_SERDES_COMMON) = 0x8000056c;

    /* delay for a while, around 20us */
    vosUDelay(10000);

    /* Enable SERDES PLL */
    /* oplRegWrite(REG_PMAC_SERDES_COMMON, 0xC000056C); */
    *(UINT32 *)((UINT32)regBaseAddr + REG_PMAC_SERDES_COMMON) = 0xC000056C;
    ...
    *(UINT32 *)((UINT32)regBaseAddr + REG_PMAC_SERDES_COMMON) = 0xE000056C;
    vosUDelay(10000);
