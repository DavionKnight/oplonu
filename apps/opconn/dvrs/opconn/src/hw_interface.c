...
extern UINT8 *regBaseAddr;
VOS_MUTEX_t g_pstRegAccessLock;
VOS_MUTEX_t g_pstTabAccessLock;

#define OPL_REG_ACCESS_LOCK vosMutexTake(&g_pstRegAccessLock)
#define OPL_REG_ACCESS_UNLOCK vosMutexGive(&g_pstRegAccessLock)

#define OPL_TAB_ACCESS_LOCK vosMutexTake(&g_pstTabAccessLock)
#define OPL_TAB_ACCESS_UNLOCK vosMutexGive(&g_pstTabAccessLock)
...
{
    if (VOS_MUTEX_OK != vosMutexCreate(&g_pstRegAccessLock))
    { 
        OPL_DRV_PRINTF(("oplDriverInit: create register access mutex failed.\r\n"));
        return OPL_ERROR;
    } 
    if (VOS_MUTEX_OK != vosMutexCreate(&g_pstTabAccessLock))
    { 
        OPL_DRV_PRINTF(("oplDriverInit: create table access mutex failed.\r\n"));
        return OPL_ERROR;
    } 
    if (OPL_OK != mdioRegisterInit())
    { 
        OPL_DRV_PRINTF(("oplDriverInit: initialize mdio register failed.\r\n"));
        return OPL_ERROR;
    } 
    return OPL_OK;
}
...
{
	vosMutexTake(&g_pstRegAccessLock);
}

void oplRegAccessUnLock(void)
{
	vosMutexGive(&g_pstRegAccessLock);
}
...
OPL_STATUS oplRegRead(UINT32 regAddr,UINT32 *regVal)
{
    ...
    OPL_REG_ACCESS_LOCK;
	*regVal = *(UINT32 *)((UINT32)regBaseAddr + regAddr);	
...
OPL_STATUS oplRegWrite(UINT32 regAddr,UINT32 regVal)
{
    ...
    OPL_REG_ACCESS_LOCK;
	*(UINT32 *)((UINT32)regBaseAddr + regAddr) = regVal;
    OPL_REG_ACCESS_UNLOCK;
	return retVal;
}
OPL_STATUS oplTabRead(OPCONN_TAB_e region,UINT32 startId,UINT32 num,UINT32 *pVal)
...
{0x7005,0x00000000,0x00000000,0X00000000,0X00000000,0X00000000,0x00000000,OPL_NULL},//changed by zhangjj 2013-3-27
