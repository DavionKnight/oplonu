...
printf("in if((IPV4_PROTOCOL_IGMP......\r\n");
   ...
   /* raisecom packet processing */
   UINT8 raise_dest_mac[6] = {0x00,0x0E,0x5E,0x00,0x00,0x01};
   UINT32 onuLlid;

   ptr_eth_raisecom_hdr = (eth_header_raisecom_t *)((unsigned char *)pkt + sizeof(host_inbound_hdr_t));
   if(!memcmp(&raise_dest_mac[0], ((UINT8 *)pkt + sizeof(host_inbound_hdr_t)) ,ETH_MAC_ADDR_LEN)
   		&& (0xFEFE == ptr_eth_raisecom_hdr->protocol))
   {
		memcpy(ptr_eth_raisecom_hdr->dest, ptr_eth_raisecom_hdr->src, ETH_MAC_ADDR_LEN);

		ponMacIdHwRead(raise_dest_mac);
		memcpy(ptr_eth_raisecom_hdr->src, raise_dest_mac, ETH_MAC_ADDR_LEN);

		ponLlidHwRead(&onuLlid);			
		ptr_eth_raisecom_hdr->llid = onuLlid;

		eopl_send_to_uplink(1, (unsigned char *)pkt + sizeof(host_inbound_hdr_t), len - sizeof(host_inbound_hdr_t));
   }
...
INT16 eopl_host_send(void *pkt, UINT16 len)
{
    ...
    sts = eopl_dma_send((void *)ptr_pkt_hdr, len);
