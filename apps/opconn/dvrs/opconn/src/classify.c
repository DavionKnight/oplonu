...
inline int clsOffsetAvailableCheck(UINT8 index,UINT8 offset,UINT8 offset_off)
{
  //printf("clsOffsetAvailableCheck,index=%d,offset=%d,offset_off=%d\n", index,offset,offset_off);
...
inline int clsOffsetCalc(UINT8 *requestValid)
{
  int i, j;
  int ret;
...
{
  int i;
  printf("in clsCRInfoPrt\n");
  for (i = 0; i < CREQUEST_LEN; i++) {
    if (crinfo[i].index != 0) {
      printf("no.%d:index=%d,offset=%d,offset_off=%d,mask=%x,rule=%x\n",
             i,
             crinfo[i].index,
             crinfo[i].foffset,
