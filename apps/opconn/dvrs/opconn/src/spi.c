...
#define PRINT_HELP printf("1. param filesize must < actually size");\
		printf("2. param filesize must be 12*N");
...
//	tmp_data=data;
	...
	printf("debug : set normal write command: 0x%x\n",(&cmd)->data1);
	printf("debug : write word: 0x%x\n",(&cmd)->data0);
...
OPL_STATUS spiNreadOnewordFromSlave(OPL_UINT32_t addr,OPL_UINT32_t *data)
{
	...
	printf("debug : set normal read command: 0x%x\n",(&cmd)->data1);
	...
	*data = tmp_data.data0;
	...
	printf("debug :read back data0  = 0x%x\n",tmp_data.data0);
...
/**
*@par Description
* This function test spi slave booting.
*
* @param [in] *pDDRImage
*	   the binary file used to train ddr controller.
* @param [in] *pBootImage
*	   the binary file used to boot slave chip .
*
 * @retval OPL_OK  This operation success.
*/

OPL_STATUS spiSlaveBootTest(OPL_UINT8_t *pDDRImage, OPL_UINT8_t *pBootImage)
{
	OPL_STATUS retVal;
	OPL_UINT8_t *pDdrDataBuf;
	OPL_UINT8_t *pBootDataBuf;
	OPL_UINT32_t ddrDataLen;
	OPL_UINT32_t bootDataLen;
	OPL_UINT32_t sramAddr;
	OPL_UINT32_t ramAddr;
	OPL_UINT32_t readLen;
	OPL_UINT32_t readVal;
	OPL_UINT32_t i;
	OPL_UINT32_t fd;
	OPL_UINT32_t readSize;

	/* config ram address to write boot image */
	sramAddr = 0x1F000000 + (0x3A000 * 4);
	
	//ramAddr = 0;
	ramAddr = 0xA1E00000;
#if 0 
	/* read file into buffer */
	ddrDataLen = vosFileRead(pDDRImage, &pDdrDataBuf);
	if(pDdrDataBuf == NULL)
	{
	...
	printf("DDR training image: %s\n", pDDRImage);
	fd = open (pDDRImage, 2, 0644);

	pDdrDataBuf = malloc(256*1024);
	memset(pDdrDataBuf, 0, 256*1024);

	ddrDataLen = 0;
	readSize = 0;
	while(1)
	{
		readSize = read(fd, pDdrDataBuf + ddrDataLen, 1024);
		if(readSize < 0)
		{
			printf("open ddr training image fail!\n");
			return -1;
		}
	...
	printf("------dataLen = %d----------------------\n", ddrDataLen);
	#if 0
	for(i = 0; i < ddrDataLen; i++)
	{
		printf("%.2x ", *(pDdrDataBuf+i));
		if((i % 16) == 15)
		{
			printf("\n");
		}
	...
	#endif
	printf("U-boot image: %s\n", pBootImage);
	fd = open(pBootImage, 2, 0644);

	pBootDataBuf = malloc(512*1024);
	memset(pBootDataBuf, 0, 512*1024);

	bootDataLen = 0;
	readSize = 0;
	while(1)
	{
		readSize = read(fd, pBootDataBuf + bootDataLen, 1024);
		if(readSize < 0)
		{
			printf("open u-boot image fail!\n");
			return -1;
		}
		else if(readSize < 1024)
		{	
			bootDataLen += readSize;
			break;
		}
...
#if 0
	printf("############################\n");
	bootDataLen = vosFileRead(pBootImage, pBootDataBuf);
	if(pDdrDataBuf == NULL)
	{
		printf("read file %s fail!\n", pBootImage);
		retVal = OPL_ERROR;
...
#endif
	/* write ddr traning image to DRAM */
	printf("-- begin to write ddr training  image---\n");
	spiSlaveTxDdrImage(sramAddr, pDdrDataBuf, ddrDataLen);

	/* run ddr training image */
	spiSlaveRunDdrImage();
	printf("-- reset cpu---\n");

	/* wait for ddr training finish */
	#if 1 
	while(1)
	{
		printf("wait for ddr training finish!\n");
		spiBytesRead(REG_GB_SPI_SLAVE_BOOT_CTRL, &readVal, sizeof(OPL_UINT32_t), &readLen);
		printf("loc1: reg(0x80) = %d\n", readVal);
	...
	printf("ddr training finish!\n");
	
	/* prepare for boot image transmition */
	//spiSlaveSramBufStartSet(0x1f000000 + 0x1F00);  /* offset = 1K byte */

	//spiSlaveSramPageSizeSet(0x1000); /* page size = 4K byte */
	
	/* write u-boot image */
	printf("-- begin to write u-boot image---\n");
	spiSlaveTxBootImage(ramAddr, pBootDataBuf, bootDataLen);

	/* run u-boot */
	printf("--- run u-boot -----\n");
	spiSlaveRunBootImage();
	printf("--- after run u-boot ----\n");
...
OPL_STATUS spiSlaveTxDdrImage(OPL_UINT32_t ramAddr, OPL_UINT8_t *pDataBuf, OPL_UINT32_t dataLen)
{
	...
	while(totalTxLen < dataLen)
	{
		...
		if(ret != NO_ERROR)
		{
			printf("write REG_SPI_SLAVE_ADDR fail!\n");
		}
		...
		if(ret != NO_ERROR)
		{
			printf("write REG_SPI_SLAVE_CTRL fail!\n");
		}
		...
		if(ret != NO_ERROR)
		{
			printf("write REG_SPI_SLAVE_DATA fail!\n");		
		}
		...
		/* write spi slave data register */
		#if 0 
		printf("txdata = %.2x %.2x %.2x %.2x\n", *(pDataBuf + totalTxLen), *(pDataBuf + totalTxLen + 1),
		                                         *(pDataBuf + totalTxLen + 2), *(pDataBuf + totalTxLen + 3));
		#endif
...
OPL_STATUS spiSlaveRunDdrImage(void)
{
	...
	if(retVal != NO_ERROR)
	{
		printf("read REG_SPI_SLAVE_CTRL fail!\n");
		return retVal;
	}
...
/**
*@par Description
* This function run slave booting code .
*
 *
 * @retval OPL_OK  This operation success.
*/

OPL_STATUS spiSlaveRunBootImage(void)
{
	OPL_UINT32_t data;
	OPL_UINT32_t txLen;
	OPL_STATUS retVal;
	...
	printf("enter spiSlaveRunBootImage() \n");
#if 1
	spiBytesRead(REG_GB_SPI_SLAVE_BOOT_CTRL, &readVal, sizeof(OPL_UINT32_t), &readLen);
	printf("loc2: reg(0x80) = %d\n", readVal);
#endif
	...
	/* write spi slave control reg */
	retVal = spiBytesWrite(REG_GB_SPI_SLAVE_BOOT_CTRL, (OPL_UINT8_t *)&data, sizeof(data), &txLen);
	if(retVal != NO_ERROR)
	{
		return retVal;
	}
