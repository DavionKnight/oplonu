...
/* defines */ 
#if 0
#define i2cInterruptClear()     {UINT32 u32tt; oplRegRead/*BitClear*/ (REG_I2C_CON, /*REG_BIT_I2CCON_SI*/(UINT32 *)&u32tt); if (nI2cDbgFd) printf ("Clear SI Interrupt ok.\n"); }
#else
...
OPL_STATUS i2cpr16 (UINT8 slave,UINT16 regisd)
{
    ...
    printf ("Read [0x%02x]EPROM address at 0x%02x%02x, value= 0x%02x.\n",(OT_EPROM_I2C_ADDR | slave), u8Buf[0],u8Buf[1], pVal);
    return OPL_OK;
}
...
OPL_STATUS i2cpw16 (UINT8 slave,UINT16 regisd, UINT8 val)
{
    ...
    printf ("Write [0x%02x]EPROM address at 0x%02x%02x, value= 0x%02x.\n",(OT_EPROM_I2C_ADDR | slave), u8Buf[0],u8Buf[1], (UINT32)val);
    return OPL_OK;
}
...
void ziic(int nLoop, int nByte, UINT8 * pDataBuf)
{
    ...
    printf ("Finished at %d/%d times, status=0x%02x.\n", k, nLoop, nRet);
}
...
int micro_read (int nBlock, int nOffset, int nByte, char * pDataBuf)
{
    ...
    if (nRet != 1) 
    {
        printf ("Error occured when write the offset to EEPROM chip......., :-(\n");
        return OPL_ERROR;
    }
...
void zmod(int nLoop, int nByte, UINT8 * pDataBuf)
{
    ...
    for (; nMod[k]; k++)
    {
        ...
        oplRegRead(REG_I2C_CON, &regVal);
        printf ("test clock mode of 0x%02x, interrupt mode.\n", regVal);
        nRet = zmic (nLoop, nByte, pDataBuf);
        if (nRet != nByte) 
        {
            printf ("Chu Cuo La......., at mode 0x%02x:-o\n", nMod[k]);
            i2cInit (1, 1, 0, k);
            break; 
        }
    }
    ...
    for (k=0; nMod[k]; k++)
    {
        oplRegWrite(REG_I2C_CON, nMod[k]);
        oplRegRead(REG_I2C_CON, &regVal);
        printf ("test clock mode of 0x%02x, Polling mode.\n", regVal);
        nRet = zmic (nLoop, nByte, pDataBuf);
        if (nRet != nByte) 
        {
            printf ("Chu Cuo La......., at mode 0x%02x:-o\n", nMod[k]);
