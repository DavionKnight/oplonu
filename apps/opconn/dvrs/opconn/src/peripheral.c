...
#define OPL_MDIO_ACCESS_LOCK vosMutexTake(&g_pstMidoAccessLock)
#define OPL_MDIO_ACCESS_UNLOCK vosMutexGive(&g_pstMidoAccessLock)

//int g_nDisableMDIO = 0;
static VOS_MUTEX_t g_pstMidoAccessLock;

//#define MDIO_LOCK_DEBUG 
#if 0
static int g_nLockNum = 0;
#define vosMutexTake(lock) { \
    int ii = 0; \
    while (1) {\
        if (pthread_mutex_trylock(lock)) { \
            printf("\n %s[%d] try get locker[%d]....%d", __FUNCTION__, __LINE__, g_nLockNum, ii); \
            usleep(10000); \
            ii++; \
        } else { \
            break; \
...
OPL_STATUS mdioRegisterInit()
{
	...
	ret =  vosMutexCreate(&g_pstMidoAccessLock);
	if (VOS_MUTEX_OK != ret)
    { 
        OPL_DRV_PRINTF(("oplDriverInit: create mdio access mutex failed.\r\n"));
        return OPL_ERROR;
    } 
