...
**    FILE       :  rstp_in.c
**      
**    DESCRIPTION:  This file contains API from an operation system to the RSTP 
**    library.
**    
**
**    NOTES      :  None.
...
//rzhou debug int max_port = 1024;
int max_port = NUMBER_OF_PORTS;

/* begin added by jiangmingli for tw_bug 2066 */
unsigned int abPortLoop[NUMBER_OF_PORTS];
unsigned int bOnePortLoopDetectEn[NUMBER_OF_PORTS];
unsigned int bOnePortLoopWhile[NUMBER_OF_PORTS];
unsigned int bOnePortLooptimer[NUMBER_OF_PORTS];
...
stp_in_stpm_create (int vlan_id, char* name, BITMAP_T* port_bmp, int* err_code)
{
  ...
  register STPM_T* this;
	...
	/* begin added by jiangmingli for rstp parameter check */
	if (NULL == port_bmp)
	{
		*err_code = STP_There_Are_No_Ports;
		return NULL;
	}
	/* end added by jiangmingli for rstp parameter check */
                    ...
                    UID_STP_MODE_T admin_state)
{
  ...
  OP_DEBUG(DEBUG_LEVEL_DEBUGGING, "_stp_in_stpm_enable debug end 01\n");
#endif
...
#ifdef __OPSP_DEBUG__
              OP_DEBUG(DEBUG_LEVEL_DEBUGGING, "_stp_in_stpm_enable debug end 02\n");
#endif

            STP_OUT_set_hardware_mode (stpm->vlan_id, STP_DISABLED);

#ifdef __OPSP_DEBUG__
              OP_DEBUG(DEBUG_LEVEL_DEBUGGING, "_stp_in_stpm_enable debug end 03\n");
#endif

            STP_stpm_enable (stpm, STP_DISABLED);

#ifdef __OPSP_DEBUG__
              OP_DEBUG(DEBUG_LEVEL_DEBUGGING, "_stp_in_stpm_enable debug end 04\n");
#endif
...
#ifdef __OPSP_DEBUG__
    OP_DEBUG(DEBUG_LEVEL_DEBUGGING, "_stp_in_stpm_enable debug end 05\n");
#endif

	/* begin modified by jiangmingli for RSTP, 2008-10-24 */
	/* if rstp instance hasn't been created, creat instance whatever the admin_state is */
	#if 0
...
#ifdef __OPSP_DEBUG__
        printf("_stp_in_stpm_enable debug end 06\n");
#endif
      this = stp_in_stpm_create (vlan_id, name, port_bmp, &err_code);

#ifdef __OPSP_DEBUG__
        printf("_stp_in_stpm_enable debug end 07\n");
#endif
...
#ifdef __OPSP_DEBUG__
    OP_DEBUG(DEBUG_LEVEL_DEBUGGING, "_stp_in_stpm_enable debug end 08\n"); 
#endif

  /* set rstp state admin_state */
  rc = STP_stpm_enable (this, admin_state);

#ifdef __OPSP_DEBUG__
    OP_DEBUG(DEBUG_LEVEL_DEBUGGING, "_stp_in_stpm_enable debug end 09\n");
#endif
...
#ifdef __OPSP_DEBUG__
  	  OP_DEBUG(DEBUG_LEVEL_DEBUGGING, "_stp_in_stpm_enable debug end 10\n");
#endif
...
#ifdef __OPSP_DEBUG__
    OP_DEBUG(DEBUG_LEVEL_DEBUGGING, "_stp_in_stpm_enable debug end 11\n");
#endif
 ...
 #ifdef __OPSP_DEBUG__
      OP_DEBUG(DEBUG_LEVEL_DEBUGGING, "_stp_in_stpm_enable debug end 12\n");
 #endif
...
#ifdef __OPSP_DEBUG__
  OP_DEBUG(DEBUG_LEVEL_DEBUGGING, "_stp_in_stpm_enable debug end 13\n");
#endif
...
*/
  ...
  OP_DEBUG(DEBUG_LEVEL_DEBUGGING, " STP_port_init called 00 \n");  
  STP_port_init (port, stpm, False);
  //rzhou 
  OP_DEBUG(DEBUG_LEVEL_DEBUGGING, " STP_port_init called 01 \n");
  //rzhou add 
  port->portEnabled = enable;
  /* rzhou del port->reselect = True;
  port->selected = False;  */
}
...
STP_IN_check_bpdu_header (BPDU_T* bpdu, size_t len)
{
  ...
  if (len8023 + 14 > (unsigned short)len) { /* len8023 format gt len :( */
...
STP_IN_rx_bpdu (int vlan_id, int port_index, BPDU_T* bpdu, size_t len)
{
  ...
  if (! this) { /*  the stpm had not yet been created :( */
    RSTP_CRITICAL_PATH_END;
	opl_rstp_printf(("Can't find rstp instance.\n"));
    return STP_Vlan_Had_Not_Yet_Been_Created;
  }

  if (STP_DISABLED == this->admin_state) {/* the stpm had not yet been enabled :( */
    RSTP_CRITICAL_PATH_END;
	opl_rstp_printf(("Rstp is disabled.\n"));
    return STP_Had_Not_Yet_Been_Enabled_On_The_Vlan;
  }

	/* begin: add by jiangmingli for RSTP process */
	#if 0
	if ((bpdu->body.message_age[0] > bpdu->body.max_age[0]) ||
		((bpdu->body.message_age[0] == bpdu->body.max_age[0]) && (bpdu->body.message_age[1] > bpdu->body.max_age[1])))
	{
		RSTP_CRITICAL_PATH_END;
  ...
  if (! port) {/* port is absent in the stpm :( */
    ...
    RSTP_CRITICAL_PATH_END;
	opl_rstp_printf(("Port %d not in the rstp instance.\n", (int)port_index));
    return STP_Port_Is_Absent_In_The_Vlan;
  }
  ...
  if (! port->portEnabled) {/* port link change indication will come later :( */
  	opl_rstp_printf(("Port %d not Enabled.\n", (int)port->port_id));
...
*/
  ...
  opl_rstp_printf(("Port %d rcv bpdu.\n", (int)port->port_id));
...
STP_IN_one_second (void)
{
  ...
  int i;
  UINT32 odmPhyStatus = 0;
  UINT32 odmPortStatus = 0;

  RSTP_CRITICAL_PATH_START;
  tev = RSTP_PORT_TIME_T; INCR100(nev);
  //printf("STP_IN_one_second called \n");
  
  for (stpm = STP_stpm_get_the_list (); stpm; stpm = stpm->next) {
    if (STP_ENABLED == stpm->admin_state) {
      /* stp_trace ("STP_IN_one_second vlan_id=%d", (int) stpm->vlan_id); */
      //printf("STP_IN_one_second: STP_stpm_one_second called \n");
      /* timers minus one, update state machine */
      ...
      {
        abPortLoop[i] = 0;
        odmPhyAdminStateGet(i+1, &odmPhyStatus);
        odmPortAdminGet(i+1, &odmPortStatus);
  
        if ((odmPhyStatus == 0) && (bOnePortLoopDetectEn[i] == 1))
        {
          if (bOnePortLooptimer[i] > 0)
          {
            bOnePortLooptimer[i]--;
          }
          if (bOnePortLooptimer[i] == 0)
          {
            //printf("STP_IN_one_second: odmPhyAdminStateSet called, bOnePortLooptimer[i] == 0 %d \n", odmPortStatus);
            odmPhyAdminStateSet(i+1, odmPortStatus);
			rstpwarningstate[i] = 0;
          }
        }
        else if ((odmPhyStatus == 0) && 
            (bOnePortLoopDetectEn[i] == 0) && 
            odmPortStatus != odmPhyStatus )
        { 
          //printf("STP_IN_one_second: odmPhyAdminStateSet called, (odmPhyStatus == 0) && (bOnePortLoopDetectEn[i] == 0) && odmPortStatus(%d) != odmPhyStatus(%d) && port(%d) \n", odmPortStatus, odmPhyStatus, i);
          odmPhyAdminStateSet(i+1, odmPortStatus);
        }
      }
      /* end added by jiangmingli for tw_bug 2066 */
                     ...
                     IN UID_STP_CFG_T* uid_cfg)
{
...
#ifdef __OPSP_DEBUG__
  OP_DEBUG(DEBUG_LEVEL_DEBUGGING, "STP_IN_stpm_set_cfg debug 01\n");
#endif

  //rzhou if in config state and STP is disabled ,then do disable all the ports
  if ((BR_CFG_STATE & uid_cfg->field_mask) &&
      (STP_DISABLED == uid_cfg->stp_enabled)) {
    /* create rstp instance, and set rstp state STP_DISABLED */  
...
#ifdef __OPSP_DEBUG__
    OP_DEBUG(DEBUG_LEVEL_DEBUGGING, "STP_IN_stpm_set_cfg debug 02\n");
#endif
  ...
  /* no such rstp instance, create rstp instance, and set rstp state STP_ENABLED */
  if (! this) { /* it had not yet been created */
  	//rzhou debug
...
#ifdef __OPSP_DEBUG__
  OP_DEBUG(DEBUG_LEVEL_DEBUGGING, "STP_IN_stpm_set_cfg debug 03\n");
#endif
  ...
  /* rstp instance already exists, but state is STP_DISABLED, convert state to  STP_ENABLED */
  if ((BR_CFG_STATE & uid_cfg->field_mask) &&
      STP_DISABLED != uid_cfg->stp_enabled &&
      STP_DISABLED == this->admin_state) {

#ifdef __OPSP_DEBUG__
  	OP_DEBUG(DEBUG_LEVEL_DEBUGGING, "STP_IN_stpm_set_cfg debug 030\n");
#endif
      	
    rc = _stp_in_stpm_enable (vlan_id, uid_cfg->vlan_name, port_bmp, uid_cfg->stp_enabled);

#ifdef __OPSP_DEBUG__
      	OP_DEBUG(DEBUG_LEVEL_DEBUGGING, "STP_IN_stpm_set_cfg debug 031\n");
#endif
...
#ifdef __OPSP_DEBUG__
    OP_DEBUG(DEBUG_LEVEL_DEBUGGING, "STP_IN_stpm_set_cfg debug 04\n");
#endif

  if (! enabled_here && STP_DISABLED != this->admin_state) {
    //rzhou change the state of all ports, but ?? 
    STP_stpm_update_after_bridge_management (this);
  }
  RSTP_CRITICAL_PATH_END;

#ifdef __OPSP_DEBUG__
  OP_DEBUG(DEBUG_LEVEL_DEBUGGING, "STP_IN_stpm_set_cfg debug 05\n");
#endif
...
#ifdef STP_DBG
      //rzhou del if (port->edge->debug) {
      if(1){ //rzhou add for test
