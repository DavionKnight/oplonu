...
void eopl_get_onu_eth_autoneglocal(void)
{
	...
	/*Support 100BASE_T2FD, 1000BASE_T, 1000BASE_TFD*/
		p_local->number = 7;
	...
	/*should use the real value, later on*/
		...
		p_local->fifth = 0x00000139;//FDX-APAUSE
		p_local->sixth = 0x0000013A;//FDX-SPAUSE
		p_local->seventh = 0x00000028;//1000BASE-T
...
void eopl_get_onu_eth_autonegadv(void)
{
	...
	/*should use the real value, later on*/
		...
		p_adv->fifth = 0x00000139;//FDX-APAUSE
		p_adv->sixth = 0x0000013A;//FDX-SPAUSE
		p_adv->seventh = 0x00000028;//1000BASE-T
		p_adv->eighth = 0x00000192;//1000BASE-TFD
#elif defined(ONU_4PORT_AR8228)||defined(ONU_4PORT_AR8327)
	/*Support 100BASE_T2FD*/
		p_adv->number = 5;
		p_adv->width = 4+p_adv->number*4;
...
//
	...
	{
		portS = 1;
#if 0  //here should be 6 ports ,1,2,3,4,cpu,ge all port should be set!!!
		portE = OAM_MAX_INSTANCE_ID;
#else
		portE = 6;
#endif
...
//				goto response;
	...
	else if(OAM_CTC_VLAN_TRANSLATE == p_vlan->mode)
	{
		...
		for(i = 0; i < (p_vlan->width - 5)/8; i++)
		{
			...
			if (vlanIndex > 16)
			{
		    	OP_DEBUG(DEBUG_LEVEL_DEBUGGING, "default tag exist in vlan list that be translate.\n", vlanId);
				ret = OAM_SET_BAD_PARAMETERS;
				goto response;
			}
	...
	else if(OAM_CTC_VLAN_AGGREGATION == p_vlan->mode)
	{
		...
		for(i = 0; i < num_of_aggr_table; i++)
		{
			...
			if ( dst_vlan == vlanId)
			{
		    	OP_DEBUG(DEBUG_LEVEL_DEBUGGING, "default tag exist in vlan list that be aggregation.\n", dst_vlan);
				ret = ERR_VARNORESOURCE;
				goto response;	
			}
