  ...
  uint8 *queryPkt)
{
  ...
  if(vosIPAddrGet("eth0", hostSrcIp) != NO_ERROR) {
...
{
  uint8 untagPacket[BIG_BUF_LEN];
  uint8 tagPacket[BIG_BUF_LEN];
  uint32 untagPacketLength;
  eth_header_with_sgl_tag_t *sglEthHeader;
  ...
  vosMemSet(untagPacket, 0, sizeof(untagPacket));
  untagPacketLength = 0;
  sglEthHeader = NULL;
  mcVid = userVid = 0;
  ret = 0;
  tmpPacketPointer = NULL;
  tmpPacketLength = 0;
  ...
  if((egressPortId!=PORT_PON_LOGIC
    ...
    /* remove tag */
    vosMemCpy(untagPacket, packet, 12);
    vosMemCpy(untagPacket+12, packet+16, packetLen-16);
    untagPacketLength = packetLen-4;

    tmpPacketPointer = untagPacket;
    tmpPacketLength = untagPacketLength;
  }
    ...
    vosMemCpy(tagPacket, packet, packetLen);
    sglEthHeader = (eth_header_with_sgl_tag_t *)tagPacket;
    if(ret == OPL_OK)
    {
    ...
    tmpPacketPointer = tagPacket;
    tmpPacketLength = packetLen;
  }
  else {
    tmpPacketPointer = packet;
    tmpPacketLength = packetLen;
  }
  ...
  uint8 outboundPacket[BIG_BUF_LEN];
  uint32 outboundPacketLength;

  out_hdr = NULL;
  vosMemSet(outboundPacket, 0, sizeof(outboundPacket));
  outboundPacketLength = 0;

  /************************** set header **************************/
  out_hdr = (host_outbound_hdr_t *)outboundPacket;
  out_hdr->reserved = 0;
  out_hdr->icos = 0;
  if(egressPortId==PORT_PON_LOGIC) { /* send to pon */
    out_hdr->eport = PORT_PON_PHYSICAL;
  }
  else {
    out_hdr->eport = PORT_GE_PHYSICAL;
  }

  /************************** copy packet **************************/
  vosMemCpy(outboundPacket+sizeof(host_outbound_hdr_t), tmpPacketPointer, tmpPacketLength);

  outboundPacketLength = tmpPacketLength+sizeof(host_outbound_hdr_t);

  MC_DEBUG(DEBUG_LEVEL_DEBUGGING, "Snooping output: send packet to %d, packetLength=%d.\r\n", egressPortId, outboundPacketLength-1);
  pktPrintDebug(outboundPacket, outboundPacketLength);

  /************************** send to cpu **************************/
  eopl_host_send((void *)out_hdr, outboundPacketLength);

  return OPL_OK;

#else /* marvell, asseros */

  MC_DEBUG(DEBUG_LEVEL_DEBUGGING, "Snooping output: send packet to %d.\r\n", egressPortId);
  pktPrintDebug(tmpPacketPointer, tmpPacketLength);

  if(egressPortId==PORT_PON_LOGIC) { /* send to pon */
    /************************** send to pon **************************/
    eopl_send_to_uplink(egressPortId, tmpPacketPointer, tmpPacketLength);
  }
  else {
    /************************** send to host **************************/
    eopl_send_to_down_link(egressPortId, 0, 2, tmpPacketPointer, tmpPacketLength);
  }
  ...
  uint32 isLastQueryPacket)
{
  ...
  MC_DEBUG(DEBUG_LEVEL_DEBUGGING, "CTC output: send packet to %d.\r\n", egressPortId);
  pktPrintDebug(tmpPacketPointer, tmpPacketLength);
  if(egressPortId==PORT_PON_LOGIC) { /* send to pon */
    /************************** send to pon **************************/
    eopl_send_to_uplink(egressPortId, tmpPacketPointer, tmpPacketLength);
...
OPL_STATUS mcCtcInput(MC_PACKET_DESC_t *desc)
{
  ...
  switch(desc->igmpType) {
    case IGMP_TYPE_GENERAL_QUERY:
      mcCtcQueryAction(desc);
      break;
    case IGMP_TYPE_V1_REPORT:
      mcControlStats.joinReportV1RxCount++;
      mcCtcJoinAction(desc);
      break;
    case IGMP_TYPE_V2_REPORT:
      mcControlStats.joinReportV2RxCount++;
      mcCtcJoinAction(desc);
