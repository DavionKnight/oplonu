...
extern OPL_CNT_t g_astCntCleHit[CNT_CLEHIT_NUM];
...
int odmAggMacAgingTimer(void)
{
	...
	for (i=1; i<ODM_MAX_NUM_OF_AGGRE_MAC; i++){
		if (vlanAggArray[i].used){
			...
			if (vosMemCmp(&(g_astCntCleHit[ruleId].stAdded), &(vlanAggMacAgeInfo[i].ruleCnt), sizeof(OPL_CNT_ADD_t))==0){				
				...
				vosMemCpy(&(vlanAggMacAgeInfo[i].ruleCnt), &(g_astCntCleHit[ruleId].stAdded), sizeof(OPL_CNT_ADD_t));
