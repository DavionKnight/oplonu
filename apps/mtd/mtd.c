 ...
 *
 * $Id: //depot/sw/abu/opconn/release/v3.2.1/apps/mtd/mtd.c#1 $
 *
...
image_check_brcm(int imagefd, const char *mtd)
{
	...
	if(fd < 0) {
		fprintf(stderr, "2 !!Could not open mtd device: %s, %x\n", mtd, fd);
		exit(1);
	}
...
int mtd_check(char *mtd)
{
	...
	fd = mtd_open(mtd, O_RDWR | O_SYNC);
	if(fd < 0) {
		fprintf(stderr, "3 !!Could not open mtd device: %s %x\n", mtd, fd);
		return 0;
	}
...
mtd_unlock(const char *mtd)
{
	...
	fd = mtd_open(mtd, O_RDWR | O_SYNC);
	if(fd < 0) {
		fprintf(stderr, "1 @@!!Could not open mtd device: %s, %x\n", mtd, fd);
		exit(1);
	}
...
mtd_erase(const char *mtd)
{
	...
	fd = mtd_open(mtd, O_RDWR | O_SYNC);
	if(fd < 0) {
		fprintf(stderr, "4 !!Could not open mtd device: %s, %x\n", mtd,fd);
		exit(1);
	}
...
int
mtd_write(int imagefd, FILE* fp, const char *mtd)
{
	int fd, i, result;
	size_t r, w;
	int e,filelen;
	...
	fd = mtd_open(mtd, O_RDWR | O_SYNC);
	if(fd < 0) {
		fprintf(stderr, "5 !!Could not open mtd device: %s, %x\n", mtd, fd);
		exit(1);
	}
...
int main (int argc, char **argv)
{
	...
	switch (cmd) {
		...
		case CMD_WRITE:
			...
			mtd_write(imagefd,image_fp, device);
			if (quiet < 2)
				fprintf(stderr, "\n");
			break;
	}
